...
// 067 for Open-to-Buy, 071 for Pricing / Scenario / Capex) — a drift means the
// button points at a page the orchestrator has no domains for.
const seedSql =
...
test("manifest covers all seventeen governed pages", () => {
  assert.equal(PERSPECTIVE_PAGES.length, 17);
});
