...
import { joiinConfigured, profitAndLoss } from "../../../lib/joiin-api";
import { mapReportRows } from "../../../lib/joiin-api-map";
...
async function handle(request, actor) {
  ...
  await audit({ actor, eventType: "joiin_api.refresh", objectType: "joiin_pl_entity", objectRef: months.join(","), detail: r });
...
export async function GET(request) {
  ...
  const r = await refresh(defaultMonths());
  await audit({ actor: "joiin-cron", eventType: "joiin_api.refresh", objectType: "joiin_pl_entity", objectRef: r.months.join(","), detail: r });
  return NextResponse.json({ ok: true, ...r });
}
