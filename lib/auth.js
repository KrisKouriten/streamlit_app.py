...
import { createSession, getSessionRow, revokeSession, revokeAllForUser, revokeOthersForUser, touchSession } from "./sessions";
...
async function signToken({ sid, user, roles, mustChange = false }) {
  const claims = { sid, uid: user.id, name: user.name, email: user.email, roles };
  // Only stamp the flag when set, so ordinary tokens stay unchanged.
  if (mustChange) claims.mustChange = true;
...
// opts.mustChange marks a session that must set a new password before it can
// use the app (admin-issued starter password / first sign-in) — the root
// middleware enforces the redirect from the token claim.
export async function startSession(user, roles = [], meta = {}, opts = {}) {
  const sid = await createSession({ userId: user.id, ip: meta.ip, userAgent: meta.userAgent });
  return signToken({ sid, user, roles, mustChange: !!opts.mustChange });
}
...
export async function getSession() {
  ...
  return { id: row.user_id, name: row.name, email: row.email, roles, sid, mustChange: payload.mustChange === true };
}
...
// End every session for a user except one (keep the current one alive). Used on
// a self-service password change so the user is not logged out of the very
// session they made the change in, but every other device is.
export async function endOtherSessions(userId, keepSid, byEmail) {
  return revokeOthersForUser(userId, keepSid, byEmail || "self", "password_change");
}
