...
import { SEED_OBJECTIVES, zeroBasedDetail, commercialSummary, initiativeInvestment } from "./dept-initiative-rules.js";
...
export async function getDeptBudgetReport({ department = null, budgetId = null } = {}) {
  try {
    const id = budgetId || (department ? await latestBudgetIdForDept(department) : await latestBudgetIdAny());
    if (!id) return { ready: false, reason: department ? `No budget found for ${department}.` : "No departmental budget has been created yet." };
    ...
    return {
      ready: true,
      budget: { budgetId: id, department: budget.department, year: budget.budget_year, version: budget.version_label, status: budget.status },
      summary: {
        target, planned,
