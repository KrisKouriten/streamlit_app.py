...
export async function getDeptBudgetReport({ department = null, budgetId = null } = {}) {
  try {
    const id = budgetId || (department ? await latestBudgetIdForDept(department) : null);
    if (!id) return { ready: false, reason: department ? `No budget found for ${department}.` : "No budget selected." };
