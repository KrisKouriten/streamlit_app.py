...
export async function getBudget(budgetId) {
  try {
    ...
    // commentary arrives with migration 050 — degrade cleanly if not applied yet.
    let lines;
    try {
      ({ rows: lines } = await query(
        `SELECT line_id, category, line_label, sort_order, prior_year, commentary, ${MONTH_KEYS.join(", ")}
         FROM finance.dept_budget_line WHERE budget_id = $1 ORDER BY sort_order, line_id`, [budgetId]));
    } catch (e) {
...
export async function saveLines(budgetId, lines = [], actor) {
  ...
  // commentary column arrives with migration 050 — include it when present.
  let hasCommentary = true;
  try { await query(`SELECT commentary FROM finance.dept_budget_line LIMIT 0`); } catch (e) { if (e?.code === "42703") hasCommentary = false; else throw e; }
  let order = 0;
  const monthCols = MONTH_KEYS.join(", ");
  for (const l of lines) {
    const base = [budgetId, (l.category || "General").trim(), String(l.line_label).trim(), l.sort_order ?? order, num(l.prior_year)];
    const months = MONTH_KEYS.map((k) => num(l[k]));
    if (hasCommentary) {
      const ph = MONTH_KEYS.map((_, i) => `$${7 + i}`).join(", ");
      await query(
        `INSERT INTO finance.dept_budget_line (budget_id, category, line_label, sort_order, prior_year, commentary, ${monthCols})
         VALUES ($1,$2,$3,$4,$5,$6,${ph})`,
        [...base, String(l.commentary || "").trim() || null, ...months]
      );
    } else {
      const ph = MONTH_KEYS.map((_, i) => `$${6 + i}`).join(", ");
      await query(
