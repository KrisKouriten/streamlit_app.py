...
// Parse a Joiin custom-report (board pack) markdown table (columns = months)
// into ordered rows: { seq, kind, label, values:{ym:number}, isPct }. kind ∈
// section | line | total | computed | pct. Section headers carry "-"; a "Total"
// row is the section subtotal (relabelled with the section it closes); indent-0
// rows with a value are computed lines (Gross Profit, EBITDA, Group Net Profit);
// anything with "%" is a ratio (stored as a fraction). Pure — no DB.
export function parseBoardPack(markdown) {
  const lines = String(markdown).split("\n").filter((l) => l.trim().startsWith("|"));
  if (!lines.length) return { months: [], rows: [] };
  const cells = (l) => l.replace(/^\s*\|/, "").replace(/\|\s*$/, "").split("|");
  ...
  for (let i = 0; i < lines.length; i++) {
    const c = cells(lines[i]);
    const yms = c.slice(1).map((x) => headerToYm(x));
    if (!c[0].trim() && yms.length && yms.some((y) => y)) { headerIdx = i; cols = yms; break; }
  }
  if (headerIdx < 0) return { months: [], rows: [] };
  ...
  for (let i = headerIdx + 1; i < lines.length; i++) {
    ...
    if (!label) continue;
    const rawVals = cols.map((ym, k) => (ym ? c[k + 1] : null));
    const isPct = /%/.test(rawVals.join(" ")) || label.endsWith("%");
    const allDash = rawVals.every((v) => v == null || /^\s*-?\s*$/.test(String(v)));
    const values = {};
    cols.forEach((ym, k) => {
      if (!ym) return;
      const n = parseMoney(rawVals[k]);
      if (n == null) return;
      values[ym] = isPct ? n / 100 : n;
    });

    // Markdown pads every cell with a leading space, so top-level rows sit at
    // indent 1 and account rows are more deeply indented (>= 2).
    let kind;
    if (allDash && indent <= 1) { kind = "section"; section = label; }
    else if (label.toLowerCase() === "total") kind = "total";
    else if (isPct) kind = "pct";
    else if (indent <= 1) kind = "computed";
    else kind = "line";

    rows.push({ seq: seq++, kind, label: kind === "total" ? `Total ${section}` : label, values, isPct });
  }
  return { months: cols.filter(Boolean), rows };
}
