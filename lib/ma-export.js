...
function appendTabSheet(wb, used, { tab, label, data }, period, title = TITLE) {
...
export async function buildManagementAccountsWorkbook({ year = null, period = "current" } = {}) {
  ...
  return buildWorkbook({ title: TITLE, tabs, period });
}
...
export function buildWorkbook({ title = TITLE, tabs = [], period = "current" } = {}) {
  if (!tabs.length) return null;
  const wb = XLSX.utils.book_new();
  const used = new Set();
  for (const t of tabs) appendTabSheet(wb, used, t, period, title);
