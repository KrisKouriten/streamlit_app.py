...
function appendTabSheet(wb, used, { tab, label, data }, period, title = TITLE, watermark = null) {
  const view = applyPeriod(data, period);
  const { aoa, pctRowsIdx, colCount } = buildTabAoa(view, label);

  // Title block above the table, then a review-note footer below it, with the
  // confidential download stamp last so every sheet carries the attribution.
  const subtitle = `${label} · ${PERIOD_LABEL[period] || period} · ${data.year}`;
  const head = [[title], [subtitle], []]; // rows 0,1,2
  const offset = head.length;
  const footer = watermark ? [[], [REVIEW_NOTE], [watermark]] : [[], [REVIEW_NOTE]];
...
export async function buildManagementAccountsWorkbook({ year = null, period = "current", watermark = null } = {}) {
  const tabs = await resolveAllTabs(year);
  if (!tabs.length) return null;
  return buildWorkbook({ title: TITLE, tabs, period, watermark });
}

// Generic workbook builder — one sheet per tab, for any dataset whose tabs are
// in the common { tab, label, data:{months, rows, year} } shape. The Report
// Builder (Tier 3.3) uses this with tabs from any dataset; the MA export above
// is just the fixed-scope case. Returns a Buffer, or null for no tabs.
export function buildWorkbook({ title = TITLE, tabs = [], period = "current", watermark = null } = {}) {
  if (!tabs.length) return null;
  const wb = XLSX.utils.book_new();
  const used = new Set();
  for (const t of tabs) appendTabSheet(wb, used, t, period, title, watermark);
