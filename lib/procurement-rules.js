...
export function budgetImpact(months = [], ym = null, addGbp = 0) {
  ...
  return {
    ym, budget, committed, spent, used, add, newCommitted,
...
export function requestsVsBudget(rows = [], months = [], isAwaiting = () => false, { all = false } = {}) {
  ...
  for (const r of rows) {
    ...
    const b = (byMonth[ym] ||= { ym, awaiting: 0, awaitingCount: 0, committed: 0, committedCount: 0 });
    if (isAwaiting(r)) { b.awaiting += amt; b.awaitingCount += 1; }
    else { b.committed += amt; b.committedCount += 1; }
  }
  ...
  return keys
    ...
    .map((ym) => {
      ...
      // exactly the queue.
      const headroom = budget == null ? null : budget - b.committed - spent;
      const headroomIfApproved = budget == null ? null : budget - wouldCommit - spent;
      return {
