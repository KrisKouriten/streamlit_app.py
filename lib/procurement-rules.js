...
export function facilityGbp(row = {}, rateFor = null) {
   ...
   *
   * The HSBC extract carries both: loan_amount / loan_currency for the drawing
   * itself, and payment_amount / payment_currency for the settlement. This read
   * the payment pair, and for a post-shipment buyer loan those are not the same
   * money — which is how November came to read £385,183 against a bank figure
   * of about £512,000.
   *
   * Whichever pair is used, the rule after that is the same: sterling stands as
   * it is, anything else converts at SPOT.
   */
  const useLoan = Number.isFinite(Number(row.loan_amount)) && !!Number(row.loan_amount);
  const amt = Number(useLoan ? row.loan_amount : row.payment_amount);
  const hasAmt = Number.isFinite(amt) && !!amt;
  const gbpOnRow = row.facility_payment_gbp != null && Number(row.facility_payment_gbp)
    ? Number(row.facility_payment_gbp) : null;
  // loan_currency is set from payment_currency at upload when the file omits it,
  // so a row with one currency column still resolves.
  const ccy = String((useLoan ? (row.loan_currency || row.payment_currency) : row.payment_currency) || "GBP").toUpperCase();

  // A sterling drawing has nothing to convert: the bank's figure, else the
  // payment amount, which is already GBP.
  if (!ccy || ccy === "GBP") return gbpOnRow != null ? gbpOnRow : (hasAmt ? amt : null);

  /*
   * A FOREIGN drawing converts at SPOT, even when the extract carries its own
   * GBP column.
   *
   * It used to be the other way round — the bank's GBP figure won outright and
   * the rate was only reached when the extract had none. That is why November
   * read £385,183 against c.£515,000: the extract's GBP for those drawings was
   * struck at an implied 1.78 against their USD value, where spot is 1.33. No
   * such rate exists anywhere in the app, which is exactly why it could not be
   * found — it was never a rate we held, it was arithmetic already baked into
   * the column we were trusting.
   *
   * The bank's GBP is whatever rate HSBC booked the drawing at, on whatever
   * date. Spot is the rate Finance maintain on the Exchange rates tab and the
   * one every other figure on the desk is struck at. Mixing the two put two
   * bases in one column and left the difference looking like spend.
   *
   * The extract's figure is still the fallback when the drawing cannot be
   * converted — no amount, or no spot rate for its currency — because a figure
   * on a different basis beats losing the money entirely.
   */
  if (hasAmt) {
    const rate = rateFor ? Number(rateFor(ccy)) : NaN;
    if (Number.isFinite(rate) && rate > 0) return amt / rate;
  }
  return gbpOnRow;   // null when there is nothing to fall back to
}

/*
 * How far the extract's own GBP figure sits from the spot conversion, for a
 * foreign drawing that carries both. Positive = the extract is HIGHER than spot.
 *
 * Reported rather than silently discarded. The old behaviour trusted this column
 * outright, so replacing it has to be auditable: if the bank's figures are sound
 * this is small change, and if one is struck at a rate nobody recognises it
 * shows up as a number with a name attached instead of as a month reading light.
 */
