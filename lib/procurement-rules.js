...
export function parseBudgetGridCsv(text) {
  ...
  const errors = [];
  if (!headers.length) return { records: [], errors: [{ row: 1, reason: "The file is empty" }] };

  const monthCols = headers.map((h, i) => ({ i, h, ym: parseMonthHeader(h) })).filter((c) => c.ym);
  if (!monthCols.length) {
    return { records: [], errors: [{ row: 1, reason: "No month columns found — the top row should carry the months (e.g. Sep-26, 2026-09)" }] };
  }
  // A month appearing twice would make the later column silently win.
  const seen = new Set();
  for (const c of monthCols) {
    if (seen.has(c.ym)) errors.push({ row: 1, reason: `Month ${c.h} appears more than once` });
    seen.add(c.ym);
  }
  ...
  records.forEach((rec, idx) => {
    const rowNo = idx + 2;
    // The source is whichever leading cell names one — usually the first column,
    // but a spreadsheet often has a blank or label column before it.
    let source = null;
    for (const h of headers) { source = parseBudgetSource(rec[h]); if (source) break; }
    if (!source) {
      ...
      const labels = headers.map((h) => String(rec[h] || "").trim()).filter(Boolean);
