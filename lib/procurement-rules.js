 ...
 * the order value.
 *
 * That fallback is the GROSS value (migration 116). The budget is a cash-out
 * plan and what leaves the bank is the gross invoice: we pay the supplier VAT
 * and reclaim it from HMRC later, on a different timetable. `amount_gbp` still
 * means net, so gross is net x (1 + vat_rate) — and Miniso defaults to no VAT,
 * because import VAT goes to HMRC at the border rather than to the supplier and
 * is not part of what the LC draws. See lib/vat-rules.js.
 */
export const committedValue = (p = {}) =>
  (p.committed_gbp != null ? Number(p.committed_gbp) : grossOf(p)) || 0;
...
/*
 * The months worth reading first.
 *
 * A budget runs across the whole horizon, so most rows are a budget figure and
 * a row of dashes — real, but not what anyone opened the page for. This keeps
 * the months carrying committed orders, spend, or an over-budget flag.
 *
 * If NOTHING has activity, every month is returned rather than an empty table:
 * a budget with no activity against it is then the only information there is,
 * and hiding it would read as "no data" instead of "nothing committed yet".
 */
export function monthsWithActivity(months = []) {
  const rows = months || [];
  const live = rows.filter((m) => activityOf(m) > 0 || m?.overBudget);
  return live.length ? live : rows;
}

/*
 * Every procurement source added together — one budget-vs-committed view.
 *
 * WHAT IS IN IT. The Merchandising dashboard is asked for the whole of merch
 * procurement in one table, not one table per source. That is Miniso purchases
 * plus Local purchases — and the OTB merchandising requests are already inside
 * those two, because a merch request is raised against a CHANNEL (Miniso MDS or
 * Local Purchase) and stored with the matching source. So every request is
 * counted once here: consolidating is addition, not a third set of rows.
 *
 * THE BUDGET IS THE SUM OF THE SOURCE BUDGETS. Nothing new to maintain, and the
 * consolidated row reconciles to the per-source rows by construction — if those
 * are right the total is right. A month only one source has budgeted shows that
 * source's figure; a month no source has budgeted stays null, so "no budget set"
 * is still distinguishable from "budgeted at zero".
 *
 * Variance and the over-budget flag are RE-STRUCK from the consolidated figures
 * rather than added up, because over-budget is not additive: a month £10k over
 * in Miniso and £15k under in Local is within budget overall, and summing two
 * booleans would call it over. The formula is the one summarise() uses, so the
 * consolidated row and the source rows cannot disagree about what it means.
 */
export function consolidateSummary(summary = {}) {
  const keys = Object.keys(SOURCES).filter((k) => summary && summary[k]);
  const n = (v) => Number(v) || 0;
  const byYm = new Map();
  for (const key of keys) {
    for (const m of summary[key].months || []) {
      const cur = byYm.get(m.ym) || {
        ym: m.ym, committed: 0, paid: 0, tradeSpent: 0, cashSpent: 0, spent: 0, fx: 0,
        budget: null, spentByDriver: {}, bySource: {},
      };
      cur.committed += n(m.committed);
      cur.paid += n(m.paid);
      cur.tradeSpent += n(m.tradeSpent);
      cur.cashSpent += n(m.cashSpent);
      cur.spent += n(m.spent);
      cur.fx += n(m.fx);
      if (m.budget != null) cur.budget = n(cur.budget) + Number(m.budget);
      for (const [driver, v] of Object.entries(m.spentByDriver || {})) {
        cur.spentByDriver[driver] = (cur.spentByDriver[driver] || 0) + n(v);
      }
      // What each source put into the month, so a consolidated figure can be
      // taken apart without leaving the table.
      cur.bySource[key] = { committed: n(m.committed), spent: n(m.spent), budget: m.budget ?? null };
      byYm.set(m.ym, cur);
    }
  }
  const months = [...byYm.values()]
    .sort((a, b) => (a.ym < b.ym ? -1 : a.ym > b.ym ? 1 : 0))
    .map((m) => ({
      ...m,
      variance: m.budget == null ? null : m.budget - m.committed - m.tradeSpent - m.cashSpent + m.fx,
      spentVariance: m.budget == null ? null : m.budget - m.spent,
      overBudget: m.budget != null && m.committed + m.tradeSpent + m.cashSpent - m.fx > m.budget,
      overSpent: m.budget != null && m.spent > m.budget,
    }));

  // One line per supplier across the sources they trade through — a supplier
  // that sells on both appears once, with both sources named.
  const bySupplier = new Map();
  for (const key of keys) {
    for (const sup of summary[key].suppliers || []) {
      const cur = bySupplier.get(sup.supplier) || { supplier: sup.supplier, terms_days: sup.terms_days, committed: 0, orders: 0, sources: [] };
      cur.committed += n(sup.committed);
      cur.orders += n(sup.orders);
      if (!cur.sources.includes(key)) cur.sources.push(key);
      bySupplier.set(sup.supplier, cur);
    }
  }
  const total = (field) => keys.reduce((s, k) => s + n(summary[k][field]), 0);
  return {
    sources: keys,
    months,
    suppliers: [...bySupplier.values()].sort((a, b) => b.committed - a.committed),
    totalCommitted: total("totalCommitted"),
    totalFx: total("totalFx"),
    totalBudget: total("totalBudget"),
    totalTradeSpent: total("totalTradeSpent"),
    totalCashSpent: total("totalCashSpent"),
    totalSpent: total("totalSpent"),
    unvaluedDrawings: total("unvaluedDrawings"),
  };
}
...
export function canDeleteProcurement(line = {}, { isFinance = false, isAdmin = false } = {}) {
  ...
  // A cancelled order commits nothing and is only clutter on the list, whether
  // it was cancelled before or after head-of-department sign-off. The cancel
  // and the delete are both in the audit log.
  if (line.approval_status === "CANCELLED") return { ok: true };
