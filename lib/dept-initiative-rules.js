...
export function phaseCost(cost = {}, { startMonth = 1, endMonth = 12 } = {}) {
  ...
  switch (cost.phasing) {
    ...
    case "ONEOFF": {
      ...
      const out = Array(12).fill(0); out[m - 1] = round2(num(cost.amount)); return out;
    }
    case "QUARTERLY": {
      const qEnds = [3, 6, 9, 12].filter((m) => m >= start && m <= end);
      return spreadAcross(cost.amount, qEnds.length ? qEnds : [end]);
    }
    case "EVEN":
    default:
      return spreadAcross(cost.amount, active);
  }
}
