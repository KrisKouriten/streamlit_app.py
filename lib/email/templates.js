 ...
 */
// `audience` tailors the wording for who's being pinged as a procurement request
// moves through sign-off: the department head who signs off first ("hod"), Finance
// once the head has approved ("finance"), the submitter when it's returned
// ("returned"), or the original Finance/Ops reviewer note ("review", default).
export function merchRequestEmail({ ref, submitter, channel, supplier, value, period, reason, link, audience = "review" } = {}) {
  const A = ({
    hod: {
      subject: `Procurement request to sign off: ${ref} (${gbp(value)})`,
      heading: "A procurement request needs your sign-off",
      lead: `${submitter || "A colleague"} has raised a merchandising / procurement request for your sign-off.`,
      button: "Review & sign off",
      footnote: "You're receiving this because you're the department sign-off for procurement. Approve or return it on the Procurement Requests screen; once you approve, it goes to Finance.",
    },
    finance: {
      subject: `Procurement request approved — ready for Finance: ${ref} (${gbp(value)})`,
      heading: "A procurement request is approved and ready for Finance",
      lead: `The head of department has signed off a procurement request raised by ${submitter || "a colleague"}. It's ready for Finance.`,
      button: "Open the request",
      footnote: "You're receiving this because you're Finance. Take it forward on Procurement Summary + Close.",
    },
    returned: {
      subject: `Procurement request not approved: ${ref}`,
      heading: "Your procurement request was not approved",
      lead: "The head of department returned your procurement request. Raise an updated request on the Procurement Requests screen if it should still go ahead.",
      button: "Open Procurement Requests",
      footnote: "You're receiving this because you raised this request.",
    },
    review: {
      subject: `New procurement request to review: ${ref} (${gbp(value)})`,
      heading: "A procurement request needs review",
      lead: `${submitter || "A colleague"} has raised a merchandising / procurement request.`,
  ...
  const a = A || {
    subject: `New procurement request to review: ${ref} (${gbp(value)})`,
    heading: "A procurement request needs review", lead: `${submitter || "A colleague"} has raised a merchandising / procurement request.`,
    button: "Review the request", footnote: "",
  };
  const html = shell({
    heading: a.heading,
    intro: `${a.lead}<br><br>${facts([
...
// Finance chasing a supplier invoice that has not arrived on a signed-off P.O.
// Goes to the department head and whoever raised the P.O — the people who can
// get the invoice from the supplier.
export function poInvoiceChaseEmail({ ref, supplier, department, value, approvedOn, expectedBy, daysOver, chasedBy, link } = {}) {
  const late = daysOver > 0 ? `${daysOver} day${daysOver === 1 ? "" : "s"} overdue` : "not yet received";
  const subject = `Invoice outstanding: ${ref}${supplier ? ` (${supplier})` : ""}`;
  const html = shell({
    heading: "An invoice is outstanding on your purchase order",
    intro: `Finance has not yet received the supplier's invoice for this purchase order. Invoices are expected within 15 days of the P.O being approved. Please ask the supplier to send it to Finance.<br><br>${facts([
      ["Reference", ref], ["Supplier", supplier], ["Department", department], ["Value", gbp(value)],
      ["Approved", approvedOn], ["Invoice expected by", expectedBy], ["Status", late], ["Chased by", chasedBy],
    ])}`,
    buttonLabel: "View purchase order",
    link,
    footnote: "Once the invoice is received, Finance will record it against the P.O and this reminder will stop.",
  });
  const text = textBody([
    "Finance has not yet received the supplier's invoice for this purchase order.",
    "Invoices are expected within 15 days of the P.O being approved. Please ask the supplier to send it to Finance.",
    "",
    `Reference: ${ref}`,
    supplier ? `Supplier: ${supplier}` : null,
    department ? `Department: ${department}` : null,
    `Value: ${gbp(value)}`,
    approvedOn ? `Approved: ${approvedOn}` : null,
    expectedBy ? `Invoice expected by: ${expectedBy}` : null,
    `Status: ${late}`,
    chasedBy ? `Chased by: ${chasedBy}` : null,
    "",
