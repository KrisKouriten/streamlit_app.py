...
// ---------------------------------------------------------------------------
// Monthly OTB projection (18-month horizon)
// ---------------------------------------------------------------------------
// The Executive summary shows the whole-horizon OTB pool. The monthly projection
// spreads it across the calendar: each month's Open-to-Buy is the classic retail
// roll — planned cost of sales + the change in target stock cover, net of the
// stock already on hand or committed, plus the month's store-opening / closure /
// clearance events. All pure and unit-tested.

// Add n months to a "YYYY-MM" period, returning "YYYY-MM".
...
// The list of `count` consecutive "YYYY-MM" periods starting at `start`.
export function periodRange(start, count) {
  const out = [];
  for (let i = 0; i < num(count); i++) { const p = addMonths(start, i); if (p) out.push(p); }
  return out;
}

/*
 * Project Open-to-Buy month by month for one channel across a horizon.
 *   salesByPeriod : { "YYYY-MM": planned sales £ }  (from the monthly sales plan)
 *   cosRate / grossMarginRate, targetWeeks : the version's assumptions
 *   openingStock : available inventory at the horizon start (store + warehouse + in-transit)
 *   eventsByPeriod : { "YYYY-MM": { newStore, fitout, closureTransferable, clearanceReduction, openCommitments } }
 * Returns one row per month with every component and the month's OTB, plus a total.
 * The monthly OTB roll: OTB = plannedCos + (endStock − beginStock) + newStore + fitout
 *   − closureTransferable − clearanceReduction − openCommitments, where beginStock is
 *   the prior month's target closing stock (opening inventory for the first month).
 */
export function projectChannelOtb({ periods = [], salesByPeriod = {}, cosRate = null, grossMarginRate = null, targetWeeks = null, openingStock = 0, eventsByPeriod = {} } = {}) {
  const months = [];
  let prevEndStock = round2(num(openingStock));
  let totalOtb = 0;
  for (const period of periods) {
    const sales = num(salesByPeriod[period]);
    const plannedCos = plannedCostOfSales(sales, { cosRate, grossMarginRate }) ?? 0;
    const endStock = targetStockFromWeeks(plannedCos, targetWeeks) ?? prevEndStock;
    const beginStock = prevEndStock;
    const ev = eventsByPeriod[period] || {};
    const newStore = num(ev.newStore), fitout = num(ev.fitout);
    const closureTransferable = num(ev.closureTransferable), clearanceReduction = num(ev.clearanceReduction);
    const openCommitments = num(ev.openCommitments);
    const otb = round2(plannedCos + (endStock - beginStock) + newStore + fitout - closureTransferable - clearanceReduction - openCommitments);
    months.push({
      period, sales: round2(sales), plannedCos, beginStock: round2(beginStock), endStock: round2(endStock),
      stockChange: round2(endStock - beginStock), newStore: round2(newStore), fitout: round2(fitout),
      closureTransferable: round2(closureTransferable), clearanceReduction: round2(clearanceReduction),
      openCommitments: round2(openCommitments), otb,
    });
    totalOtb += otb;
    prevEndStock = endStock;
  }
  return { months, totalOtb: round2(totalOtb) };
}
