...
// The nominal each payroll component posts to by default. These MUST match the
// store P&L template's staff-cost line accounts (lib/pl-format.js STORE_FORMAT)
// or the computed payroll renders as `unmapped` and drops out of the P&L. Keep
// in lockstep with the template account names.
export const DEFAULT_PAYROLL_NOMINALS = {
  basic: "ST: Salaries - Basic Pay",
  holiday: "ST: Salaries - Holiday Pay",
  pension: "ST: Pensions Costs",
  er_ni: "ST: Employers National Insurance",
};
...
export function computePayrollLines(rule = {}, rates = {}, overrides = []) {
  ...
  const noms = {
    basic: rule.nominal_basic || DEFAULT_PAYROLL_NOMINALS.basic,
    holiday: rule.nominal_holiday || DEFAULT_PAYROLL_NOMINALS.holiday,
    pension: rule.nominal_pension || DEFAULT_PAYROLL_NOMINALS.pension,
    er_ni: rule.nominal_er_ni || DEFAULT_PAYROLL_NOMINALS.er_ni,
  };
  const out = [];
