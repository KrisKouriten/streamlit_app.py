...
// SLT_APPROVAL → LOCKED, and can be returned to DRAFT at any review stage. From
// DRAFT the preparers can also submit straight to their department head
// (→ DEPT_APPROVAL), skipping Finance review.
...
// A budget is either an annual BUSINESS budget or a PROJECT budget linked to one
// Business Project. Anything else falls back to BUSINESS.
export const BUDGET_TYPES = [
  { code: "BUSINESS", label: "Business (annual)" },
  { code: "PROJECT", label: "Project" },
];
export function normBudgetType(v) {
  return String(v || "").toUpperCase() === "PROJECT" ? "PROJECT" : "BUSINESS";
}

export function validateBudget({ department, budget_year, budget_type, business_project_id } = {}) {
  ...
  // A project budget must name the Business Project it belongs to.
  if (normBudgetType(budget_type) === "PROJECT" && !(Number(business_project_id) > 0)) {
    return "Choose the Business Project this budget is for";
  }
  return null;
}
...
export const BUDGET_TRANSITIONS = {
  ...
  // Preparers (a department's own team) can send a draft straight to their
  // department head for sign-off, skipping Finance review. From Department
  // Approval the chain continues as normal (→ SLT Approval → Locked).
  submit_to_dept:    { from: ["DRAFT"], to: "DEPT_APPROVAL", role: "OWNER", label: "Submit to department head" },
