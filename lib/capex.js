...
const COLS = `project_id, name, investment_type, scenario_label, store_code, entity_id, region, opening_date,
  ...
  working_capital, contingency, other, years, year1_revenue, revenue_growth_pct, gross_margin_pct, payroll_pct,
...
export async function upsertProject(input, actor) {
  if (!input.name) throw new Error("Give the project a name");
