...
export async function getMaDashboard({ period = "current", year = null } = {}) {
  ...
  const diag = `actuals:${act.ready ? (act.loaded ? "loaded" : "empty") : "no-table"} · forecast:${fc.loaded ? "loaded" : fc.ready ? "empty" : "no-table"}`;
  if (!act.ready) return { ready: false, loaded: false, forecastLoaded: fc.loaded, diag };
  if (!act.loaded) return { ready: true, loaded: false, forecastLoaded: fc.loaded, diag };

  const built = assembleDashboard(act.byScope, forecast, period, year);
  return { ready: true, loaded: true, forecastLoaded: fc.loaded, diag, ...built };
}
