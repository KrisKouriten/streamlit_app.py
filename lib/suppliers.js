...
import { validateSupplier, summariseExposure, facilityHeadroom, normName } from "./suppliers-rules.js";
...
export async function listSuppliers({ activeOnly = false } = {}) {
  ...
  const map = (rows) => rows.map((r) => ({
    ...
    pending_details: r.pending_details === true,
  }));
  try {
    // Prefer the migration-100 column (pending_details), then the migration-092
    // columns (payment_days, active_merch); fall back to the base set as each
    // migration may not yet be applied.
    try {
      const { rows } = await query(
        `SELECT supplier_id AS id, name, norm_name, source_type, currency, credit_limit, payment_days, active, active_merch, pending_details
           FROM finance.supplier ${where} ORDER BY name`);
      return { ready: true, suppliers: map(rows) };
    } catch (e0) {
      if (e0?.code !== "42703") throw e0;   // pre-100: no pending_details
      try {
        const { rows } = await query(
...
export async function upsertSupplier(input, actor) {
  ...
  if (clean.id) {
    ...
    try {
      try {
        // post-100: Finance saving completes the record, so clear the stub flag.
        await query(
          `UPDATE finance.supplier SET name=$2, norm_name=$3, source_type=$4, currency=$5, credit_limit=$6, payment_days=$7, active=$8, active_merch=$9, notes=$10, pending_details=false, updated_by=$11, updated_at=CURRENT_TIMESTAMP
             WHERE supplier_id=$1`,
        ...
        if (!columnMissing(e0)) throw e0;   // pre-100: no pending_details
        try {
          await query(
  ...
  let rows;
  try {
    // post-100: a Finance-entered supplier is complete (pending_details=false);
    // re-adding a stub name here also clears its "awaiting details" flag.
    ({ rows } = await query(
      `INSERT INTO finance.supplier (name, norm_name, source_type, currency, credit_limit, payment_days, active, active_merch, notes, pending_details, created_by, updated_by)
       ...
       ON CONFLICT (norm_name) DO UPDATE SET name=EXCLUDED.name, source_type=COALESCE(EXCLUDED.source_type, finance.supplier.source_type),
         credit_limit=EXCLUDED.credit_limit, currency=EXCLUDED.currency, payment_days=EXCLUDED.payment_days, active=EXCLUDED.active, active_merch=EXCLUDED.active_merch, notes=EXCLUDED.notes, pending_details=false, updated_by=EXCLUDED.updated_by, updated_at=CURRENT_TIMESTAMP
...
/*
 * Quick-add a supplier from a request form (Procurement Request / Purchase Order).
 * Anyone raising a request may add a supplier that is not yet on the master; it is
 * created as a stub (name only) flagged pending_details so Finance completes its
 * credit limit, payment terms and source classification on Suppliers & Credit.
 * If the name already exists (by normalised name) that supplier is returned — no
 * duplicate is created — so typing an existing name simply selects it.
 */
export async function proposeSupplier(name, actor) {
  const clean = String(name == null ? "" : name).trim().replace(/\s+/g, " ");
  if (!clean) throw new Error("Enter a supplier name");
  if (clean.length > 160) throw new Error("Supplier name is too long (160 characters max)");
  const nn = normName(clean);
  // Already on the master? Return it rather than duplicating.
  const existing = await query(`SELECT supplier_id AS id, name FROM finance.supplier WHERE norm_name = $1`, [nn]);
  if (existing.rows.length) return { id: existing.rows[0].id, name: existing.rows[0].name, existed: true };
  let rows;
  try {
    ({ rows } = await query(
      `INSERT INTO finance.supplier (name, norm_name, active, active_merch, pending_details, created_by, updated_by)
       VALUES ($1,$2,true,true,true,$3,$3)
       ON CONFLICT (norm_name) DO UPDATE SET name = finance.supplier.name
       RETURNING supplier_id AS id, name`,
      [clean, nn, actor]));
  } catch (e1) {
    if (!columnMissing(e1)) throw e1;       // pre-100 / pre-092: no pending_details / active_merch
    ({ rows } = await query(
      `INSERT INTO finance.supplier (name, norm_name, active, created_by, updated_by)
       VALUES ($1,$2,true,$3,$3)
       ON CONFLICT (norm_name) DO UPDATE SET name = finance.supplier.name
       RETURNING supplier_id AS id, name`,
      [clean, nn, actor]));
  }
  await audit({ actor, eventType: "supplier.propose", objectType: "supplier", objectRef: String(rows[0].id), detail: { name: clean } });
  return { id: rows[0].id, name: rows[0].name, existed: false };
}
