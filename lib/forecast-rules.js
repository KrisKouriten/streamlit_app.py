...
function augmentLabour(byLine) {
  ...
  const holiday = {}, pension = {};
  for (const key of Object.keys(basic)) {
    const b = Number(basic[key]) || 0;
    const h = b * LABOUR.HOLIDAY_PCT;
    holiday[key] = h;
    pension[key] = (b + h) * LABOUR.PENSION_PCT;
  }
  byLine[LABOUR.HOLIDAY] = holiday;
  byLine[LABOUR.PENSION] = pension;
  // Employer NI is a per-store, per-month input (it varies with the full/part-time
  // mix). When the workbook supplies it, use it as-is; otherwise carry a nil line
  // at the fallback rate so the four labour lines are always reported together.
  if (!byLine[LABOUR.NI]) {
    const ni = {};
    for (const key of Object.keys(basic)) ni[key] = (Number(basic[key]) || 0) * (1 + LABOUR.HOLIDAY_PCT) * LABOUR.NI_PCT;
    byLine[LABOUR.NI] = ni;
  }
  return byLine;
}
