...
export function parseForecastWorkbook(wb) {
  ...
  // Sheet names differ between versions (Labour Seasonality / ST Salaries, etc.).
  const salariesName = findSheet(wb, "salar") || findSheet(wb, "labour") || findSheet(wb, "seasonal");
  const niName = wb.SheetNames.find((n) => n.trim().toLowerCase().split(/\s+/).includes("ni"));
  const pensionName = findSheet(wb, "pension");

  const labourBlocks = seasonalBlocks(salariesName);
  ...
  addOnCost(blockRows(seasonalBlocks(niName), /.*/), "ST: Salaries - Employer's NI");
  addOnCost(blockRows(seasonalBlocks(pensionName), /.*/), "ST: Salaries - Pension");

  // --- Per-nominal variable tabs (ST COGS, ST Advertising & Marketing, …) ------
  // Each is a store × month seasonal grid of % of sales for one nominal. The
  // nominal name is the block label (minus its "(% of Sales)" suffix). These
  // month-specific rates override the flat Variable Costs block for the same
  // nominal. Labour / NI / pension / structural tabs are handled above.
  const structural = new Set([salariesName, niName, pensionName].filter(Boolean));
  for (const name of wb.SheetNames) {
    const low = name.trim().toLowerCase();
    if (structural.has(name)) continue;
    if (low.includes("sales forecast") || low.includes("cost assumption") || low.includes("fixed cost")) continue;
    const blk = seasonalBlocks(name)[0];
    if (!blk || !blk.rows.length) continue;
    const label = blk.label.replace(/\s*\([^)]*\)\s*$/, "").trim(); // drop "(% of Sales)"
    if (!label) continue;
    for (const { store, mo, value } of blk.rows) {
      if (!storeEntity[store]) continue;
      emit(label, store, mo, value);
    }
  }
