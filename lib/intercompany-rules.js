...
export const CATEGORIES = {
  CASH: {
    ...
    cols: ["gross_amount", "payment_method", "reference"],
    csvTemplate: "Credit Entity (CF Out),Date,Currency,Amount,Payment Method,Payment Reference (Unique ID),Debit Entity (CF In)",
  },
  INVENTORY_RECHARGES: {
...
// The raw, quote-aware rows of a CSV, with wholly blank rows dropped. Exposed
// for parsers that can't assume the first row is the header — a spreadsheet
// export often carries a title or blank line above it.
...
/*
 * Best-effort match of a recharge store to its legal entity. Store names
 * ("Brighton", "Miniso UK — Brighton") and entity names ("Miniso UK — Brighton",
 * "Kouriten Brighton Limited") are normalised — em/en dashes to spaces, the
 * house prefix "Miniso UK", "Kouriten" and the "Limited"/"Ltd" suffix stripped —
 * then matched exact first, then by containment. Returns entity_id or null.
 * `entities` is [{ entity_id, entity_name, legal_name }]. Pure, unit-tested.
 */
export function normEntityName(s) {
  return (s || "").toLowerCase()
    .replace(/[—–-]/g, " ")
    .replace(/\blimited\b|\bltd\b|\bminiso uk\b|\bkouriten\b/g, " ")
    .replace(/[^a-z0-9 ]/g, " ")
    .replace(/\s+/g, " ").trim();
}
export function matchStoreEntity(entities, storeName) {
  const target = normEntityName(storeName);
  if (!target) return null;
  for (const e of entities || []) { if (normEntityName(e.entity_name) === target || normEntityName(e.legal_name) === target) return e.entity_id; }
  for (const e of entities || []) { const en = normEntityName(e.entity_name), ln = normEntityName(e.legal_name); if ((en && en.includes(target)) || (ln && ln.includes(target))) return e.entity_id; }
  return null;
}
