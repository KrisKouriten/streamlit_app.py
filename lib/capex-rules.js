...
export const INVESTMENT_COMPONENTS = [
  "fit_out", "fixtures", "it", "inventory", "professional_fees", "marketing", "working_capital",
  "contingency", "other",
];
// Inventory and working capital are cash tied up at t0 but are not capex, so
// they are excluded from the depreciable base. (Occupancy costs — rent, business
// rates, service charge — are recurring operating costs and are modelled as an
// annual charge in projectModel, not as part of the upfront investment.)
export const NON_DEPRECIABLE_COMPONENTS = ["inventory", "working_capital"];
// Recurring annual occupancy cost components — a model assumption, charged in the
// P&L every year (above EBITDA), never part of the upfront investment.
export const OCCUPANCY_COMPONENTS = ["rent", "business_rates", "service_charge"];

// The annual occupancy charge for a project: the sum of its occupancy assumptions
// (rent + business rates + service charge), or an explicit occupancy_annual override.
export function annualOccupancy(input = {}) {
  if (input.occupancy_annual != null && input.occupancy_annual !== "") return r2(input.occupancy_annual);
  return r2(OCCUPANCY_COMPONENTS.reduce((t, k) => t + num(input[k]), 0));
}
...
export function projectModel(input = {}) {
  ...
  const depreciable = input.depreciable_capex != null ? num(input.depreciable_capex)
    : invest - NON_DEPRECIABLE_COMPONENTS.reduce((t, k) => t + num(input.investment?.[k]), 0);
  ...
  // Occupancy (rent + business rates + service charge) — a recurring annual
  // operating cost, held flat across the model horizon, charged above EBITDA.
  const occupancy = annualOccupancy(input);
  const rows = [];
  ...
  for (let y = 1; y <= years; y++) {
    ...
    const ebitda = grossProfit - payroll - opex - occupancy;
