...
export const NAV_SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Month-End Close", href: "/operate/management-close", hint: "Pre-close checks & the reconciliation playbook" },
...
export function findModule(slug) {
  for (const s of NAV_SECTIONS) {
    for (const it of s.items) {
      if (it.slug === slug) return { ...it, section: s.label, sectionKey: s.key };
    }
  }
  return null;
}
...
export function activeHref(path) {
  ...
  for (const s of NAV_SECTIONS) {
    for (const it of s.items) {
      const h = it.href || (it.slug ? `/module/${it.slug}` : null);
