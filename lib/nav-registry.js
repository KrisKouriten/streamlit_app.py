...
// The operating departments that get their own dashboard (one engine, keyed by
// ?dept). Shared truth for the sidebar items and the per-department visibility
// rule so they always agree on the item key.
export const DEPARTMENT_DASHBOARD_DEPTS = ["Marketing", "Finance", "Merchandising", "Operations", "HR", "Logistics", "Architecture & Build"];
export function deptDashboardHref(dept) { return `/dashboards/department-budget?dept=${encodeURIComponent(dept)}`; }
const DEPARTMENT_DASHBOARD_ITEMS = DEPARTMENT_DASHBOARD_DEPTS.map((d) => ({
  label: `${d} Dashboard`, href: deptDashboardHref(d), hint: `${d} budget, P.Os, committed spend & challenges`,
}));
...
export const NAV_SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Treasury Dashboard", href: "/finance-os/treasury", hint: "Facilities, funding, hedging and forward cash", related: [["Cash Flow", "/finance-os/cashflow"]] },
      // One dashboard per department — same engine, keyed by ?dept. All of a
      // department's P.O activity (requests + close/challenge) rolls up here.
      // Non-admin/finance users only see their own (see getHiddenNavForSession).
      ...DEPARTMENT_DASHBOARD_ITEMS,
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Plan P&L (preview)", href: "/plan/pl", hint: "Read-only P&L from the driver engine — by scope, version & scenario" },
      ...
      { label: "Capex Investment", slug: "capex-investment", hint: "Appraise investments — IRR, NPV, payback & a 10-year model per project" },
      { label: "Capital Allocation", slug: "capital-allocation", hint: "Portfolio view — capital available, committed & funding required against the hurdle rate", related: [["Capex Investment", "/plan/capex"]] },
    ],
  },
  {
    ...
    items: [
      ...
      { label: "Pricing Scenario", slug: "pricing-scenario", hint: "Model promotions & price changes — margin impact before implementation" },
      { label: "Inventory Position", href: "/plan/inventory-position", hint: "Live stock position — in transit, DC and stores — feeding OTB" },
      { label: "Purchase Order Requests", href: "/operate/po-tracker", hint: "Raise a P.O and take it through department-head sign-off" },
      { label: "Procurement Requests", href: "/operate/procurement", hint: "Raise Miniso & local purchase requests; monthly cash budget vs committed spend" },
      ...
      { label: "Miscellaneous Spend", href: "/plan/misc-spend", hint: "Log small spend that doesn't need a P.O, against a budget" },
      { label: "Card / Pre-approved Spend", href: "/plan/card-spend", hint: "Log card or pre-approved spend against a budget — reports as committed spend" },
    ],
  },
  {
    ...
    items: [
      ...
      { label: "Treasury", href: "/finance-os/treasury", hint: "Bank trade facility, term loans, hedging, sales income & store cash rec.", related: [["Cash Flow", "/finance-os/cashflow"]] },
  ...
  },
  {
    ...
    items: [
      ...
      { label: "P.O Summary + Close", href: "/operate/po-summary", hint: "Finance: review P.Os, record invoices, close or challenge" },
      { label: "Procurement Summary + Close", href: "/operate/procurement-summary", hint: "Finance: approve, challenge, invoice & close procurement purchases (Miniso / Local / merch requests)" },
      { label: "Suppliers & Credit", href: "/operate/suppliers", hint: "Supplier orders vs credit limits & the HSBC facility position" },
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Exchange Rates", href: "/data/exchange-rates", hint: "USD→GBP spot / hedged / costing rates" },
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Data Quality", href: "/govern/data-quality", hint: "Whether each governed feed is actually landing \u2014 missing tables, unrecognised values, unconverted amounts" },
...
export const MODULE_FLAGS = {
  ...
  "my-finance-home": "/finance-os/my-home",
  ...
  "department-budget-dashboard": "/dashboards/department-budget",
  "budget-builder": "/plan/builder",
  "consolidated-pl": "/plan/consolidated",
  "otb-planning": "/plan/otb",
  "pricing-review": "/plan/pricing",
  "pricing-scenario": "/plan/pricing-scenario",
  "capex-investment": "/plan/capex",
  "capital-allocation": "/plan/capex/allocation",
};
...
// A human label for a live path — the matching nav item's label, else a
// title-cased last segment. Used by the recently-viewed tracker.
export function navLabelForPath(path) {
  const active = activeHref(path);
  if (active) {
    for (const s of NAV_SECTIONS) {
      for (const it of s.items) {
        if (resolveHref(it) === active) return it.label;
      }
    }
  }
  const seg = String(path || "").split("/").filter(Boolean).pop() || "";
  return seg ? seg.replace(/[-_]/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()) : "Page";
}
