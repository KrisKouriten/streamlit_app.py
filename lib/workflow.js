...
export async function getTeamWeek(weekStartIso) {
  const [tasks, capacity] = await Promise.all([
    ...
    query(
      // The weekly schedule is the FINANCE team's schedule, so only surface
      // active users whose governed department (set in Users, Roles &
      // Permissions) is Finance — non-finance staff are excluded.
      `SELECT u.id AS user_id, u.name, COALESCE(c.weekly_hours, 37.5) AS weekly_hours
       ...
       WHERE u.is_active AND lower(trim(u.department)) = 'finance' ORDER BY u.name`
    ),
  ]);
