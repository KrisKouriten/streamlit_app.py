...
export function computeAccrualReview(records = [], { targetMonth = null, materiality = DEFAULT_MATERIALITY, expectations = [] } = {}) {
  ...
  const empty = {
    ...
    basisCounts: { MODEL: 0, RUN_RATE: 0 }, modelLoaded: expectations.length > 0,
  };
  ...
  // Model expectations keyed store||nominal.
  const expMap = {};
  for (const e of expectations) expMap[(e.store || "—") + "||" + e.line_label] = e;
  ...
  for (const s of Object.values(series)) {
    ...
    const exp = expMap[s.store + "||" + s.nominal];
    let expected, basis, monthsUsed;
    if (exp) {
      basis = "MODEL";
      expected = expectedForMonth(exp, storeRevenue[s.store]?.[target] || 0);
      monthsUsed = 0;
    } else {
      const present = priorMonths.filter((m) => s.m[m] != null);
  ...
  return {
    ...
    basisCounts, modelLoaded: expectations.length > 0,
  };
}
