import { grossOf } from "./vat-rules.js";
/*
 ...
 * drawing, so a row tagged TRADE_PAY could not be tied back to the HSBC
 * facility and the two registers had to be reconciled by eye. The point of the
 * link is that a procurement order can be closed once the loan behind it is
 * settled in full.
 *
 * Normalised the same way LC references are, so "wctuka096701" and
 * " WCTUKA096701 " are the same reference.
 */
export const normTradePayRef = (s) => String(s == null ? "" : s).trim().replace(/\s+/g, "").toUpperCase();

// A reference only belongs on a row settled by trade pay. Cash has no drawing.
export function tradePayRefError(method, ref) {
  const r = normTradePayRef(ref);
  if (!r) return null;                                  // blank is always fine — it is optional
  if (method !== "TRADE_PAY") return "A trade-pay reference only applies to a purchase settled by trade pay.";
  if (r.length < 4) return "That is too short to be a drawing reference.";
  if (r.length > 40) return "That is too long to be a drawing reference.";
  if (!/^[A-Z0-9/.-]+$/.test(r)) return "A drawing reference is letters, digits and - / . only.";
  return null;
}

/*
 * How a purchase's trade-pay reference stands against the facility register.
 *
 * `facilityRefs` is the set of normalised references on finance.bank_trade_facility.
 * NOT FOUND is a warning, never an error: the extract is uploaded periodically,
 * so a genuine reference may simply not be loaded yet, and refusing it would
 * stop Finance recording a payment that really happened. The same treatment LC
 * references already get.
 */
export function tradePayMatch(row = {}, facilityRefs = null) {
  const method = row.payment_method || null;
  const ref = normTradePayRef(row.trade_pay_ref);
  if (method !== "TRADE_PAY") return { state: "n/a", ref: null, label: null, tone: null };
  if (!ref) {
    return { state: "missing", ref: null, tone: "amber",
      label: "No drawing reference — this cannot be reconciled to the facility." };
  }
  if (facilityRefs == null) {
    return { state: "unknown", ref, tone: "muted",
      label: "Facility register not loaded, so this reference cannot be checked." };
  }
  if (facilityRefs.has(ref)) {
    return { state: "matched", ref, tone: "green", label: `Reconciles to ${ref} on the facility.` };
  }
  return { state: "unmatched", ref, tone: "amber",
    label: `${ref} is not on the facility register — check the reference, or upload a newer HSBC extract.` };
}
 ...
 *
 * The LC drawdown / LC balance pair only ever meant anything for Miniso, which
 * settles by letter of credit. A Local Purchase showed a dash in both, which is
 * not "nothing to say" — it is the same question with a different answer:
 *
 *   * Not paid yet        — the whole value is still committed.
 *   * Settled in CASH     — the money has gone; nothing is outstanding.
 *   * Settled on TRADE PAY— HSBC paid the supplier and the drawing is on the
 *                           facility, which the desk ALREADY reports as spend.
 *                           Leaving it committed as well charges the month twice
 *                           for the same money — exactly the double count that
 *                           made every Miniso month read over.
 *
 * So a settled row commits nothing, whichever way it settled, and the order can
 * be closed once the drawing behind it is repaid.
 *
 * Returns { drawn, balance, note, tone, closable }:
 *   drawn    what has left, or is on the facility. Null where nothing has.
 *   balance  what is still committed. Never null except where FX makes it
 *            unknowable, because "we cannot say" and "nothing" are different.
 *   closable the facility reports the drawing fully repaid — the signal the
 *            procurement order behind it can be closed.
 */
/*
 * A Miniso order that was settled some other way than its LC — paid on TradePay
 * (a Miniso Facility drawing, recorded by its WC… reference) or in cash. It
 * then settles exactly as a Local purchase does: the money has gone, or is a
 * drawing the facility already reports as spend, so the LC balance no longer
 * describes it.
 */
export const paidOutsideLc = (row = {}) =>
  settlesByLc(row) && row.payment_status === "PAID" && (row.payment_method === "TRADE_PAY" || row.payment_method === "CASH");

export function outstandingCommitment(row = {}, costingRate = null) {
  if (settlesByLc(row) && !paidOutsideLc(row)) {
    const drawn = lcDrawdownGbp(row, costingRate);
    const balance = lcBalanceGbp(row, costingRate);
    return {
      drawn, balance,
      note: balance == null ? null : balance < 0 ? "drawn over the order value" : "still committed",
      tone: balance != null && balance < 0 ? "red" : null,
      closable: balance != null && balance <= 0,
    };
  }

  const value = committedAmount(row);
  const paid = row.payment_status === "PAID";
  if (!paid) return { drawn: null, balance: value, note: "still committed", tone: null, closable: false };

  if (row.payment_method === "CASH") {
    return { drawn: value, balance: 0, note: "settled in cash", tone: "green", closable: true };
  }
  if (row.payment_method === "TRADE_PAY") {
    // The drawing is on the facility and counted as spend there. Committing it
    // again here would be the double count.
    const settled = row.trade_pay_settled === true;
    const m = row.trade_pay || {};
    return {
      drawn: value, balance: 0,
      note: settled ? "loan repaid — ready to close"
        : m.state === "matched" ? "on the facility"
        : m.state === "missing" ? "no drawing reference"
        : m.state === "unmatched" ? "reference not on the facility"
        : "on the facility",
      tone: settled ? "green" : m.state === "matched" ? null : "amber",
      closable: settled,
    };
  }
  // Paid, but nobody has said how. Not guessed at: tagging it would move real
  // money between Cash and Trade pay on the desk.
  return { drawn: value, balance: 0, note: "paid — settlement method not recorded", tone: "amber", closable: false };
}

/*
 * What a SETTLED non-LC purchase still commits against its budget month — or
 * null where the row is not one, and its ordinary value stands.
 *
 * A Local purchase paid in cash has gone; one paid on trade pay is a drawing on
 * the facility, which every budget view already reports as spend. Either way it
 * commits nothing, and counting it as committed as well charges the month twice.
 *
 * Both budget views read this one rule. They used to decide it separately: the
 * Procurement Requests rollup zeroed a paid row, while the close desk's
 * "Awaiting your decision vs budget" panel did not — so once Finance recorded
 * trade-pay references, every paid Local order showed in Committed AND in Spent
 * there, and Dec 2026 read roughly twice over.
 */
export function settledCommitment(row = {}) {
  if (paidOutsideLc(row)) return 0;                // Miniso paid on TradePay / in cash
  if (settlesByLc(row)) return null;               // LC rows: the LC balance rule applies
  if (row.payment_status !== "PAID") return null;  // not settled — full value stands
  return outstandingCommitment(row).balance;
}
...
/*
 * The committed amount once closed — invoice net where recorded, else the line
 * value, GROSSED at the row's VAT rate (migration 116).
 *
 * What leaves the bank is the gross invoice. Both figures this chooses between
 * are net: `invoice_amount` is the invoice net Finance key in, and `amount_gbp`
 * is the net Merch entered. Miniso defaults to no VAT, so its figures are
 * unchanged — import VAT is paid to HMRC at the border, not to the supplier.
 */
export function committedAmount(row = {}) {
  const field = row.invoice_amount != null ? "invoice_amount" : (row.landed_cost != null && num(row.landed_cost) > 0 ? "landed_cost" : "amount_gbp");
  return grossOf(row, field);
}
// The same figure before VAT, for showing the two side by side.
export function committedAmountNet(row = {}) {
