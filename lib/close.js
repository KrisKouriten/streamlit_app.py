...
import { TOTAL_TASKS } from "./close-config";
...
export async function getSignals(period) {
  ...
  // 5. Month-end close checklist — the per-entity task board (Step 1 · Month-End
  //    Close) stored in public.task_state, evaluated against every active entity's
  //    full checklist (active entities × TOTAL_TASKS). This is the same completion
  //    the Month-End Close screen shows, so finishing Step 1 satisfies this gate.
  let tasks = { open: 0, overdueCritical: 0, total: 0, done: 0, entities: 0 };
  try {
    const [{ rows: dRows }, ents] = await Promise.all([
      query(`SELECT count(*) FILTER (WHERE done)::int AS done FROM public.task_state WHERE period = $1`, [period]),
      listEntities().catch(() => []),
    ]);
    const activeEntities = (ents || []).filter((e) => e.is_active).length;
    const total = activeEntities * TOTAL_TASKS;
    const done = Math.min(dRows[0].done, total || dRows[0].done);
    tasks = { total, done, open: Math.max(0, total - done), overdueCritical: 0, entities: activeEntities };
  } catch (e) { if (!tableMissing(e)) throw e; }
