...
// One merch request by id — used by the API to gate transitions and build
// notifications. Returns null when it doesn't exist (or before the table exists).
export async function getMerchRequest(id) {
  try {
...
export async function transitionMerchRequest(id, action, actor) {
  ...
  const to = {
    submit: "MERCH_REVIEW",
    // Head-of-department sign-off (the Merchandising Department sign-off approver,
    // e.g. Becky): once approved the request goes straight to Finance.
    hod_approve: "FINANCE_REVIEW",
    validate: "OTB_VALIDATED", finance: "FINANCE_REVIEW", approve: "APPROVED",
