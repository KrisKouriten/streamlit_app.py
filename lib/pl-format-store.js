...
// Which entities feed a scope kind (for the "present nominals" side of the map).
async function scopeEntityFilter(scopeKind) {
  if (scopeKind === "store") {
    const { rows } = await query(`SELECT DISTINCT entity_id FROM finance.joiin_pl_entity WHERE account = 'ST: Sales'`);
    return { clause: `entity_id = ANY($1)`, params: [rows.map((r) => r.entity_id)] };
  }
  if (scopeKind === "franchise") return { clause: `entity_name ILIKE '%franchise%'`, params: [] };
  if (scopeKind === "head_office") return { clause: `entity_name = 'Kouriten Limited'`, params: [] };
  return { clause: `TRUE`, params: [] }; // consolidated = all
}

// Mapping report for a scope: which nominals the format claims (mapped), which
// appear in the data (present), and which are present-but-unmapped — the guard
// that stops a nominal silently dropping out of a subtotal.
export async function getMappingReport(scopeKind) {
  const spec = await getFormatSpec(scopeKind);
  const lines = (spec || []).filter((e) => e.kind === "line").map((e) => ({ label: e.label, accounts: e.accounts || [] }));
  const mapped = new Set(lines.flatMap((l) => l.accounts));
  let present = [];
  try {
    const { clause, params } = await scopeEntityFilter(scopeKind);
    const { rows } = await query(
      `SELECT account, COALESCE(SUM(value),0) total FROM finance.joiin_pl_entity WHERE ${clause} GROUP BY account HAVING SUM(value) <> 0 ORDER BY account`,
      params
    );
    present = rows.map((r) => ({ account: r.account, total: Number(r.total) }));
  } catch (e) {
    if (!tableMissing(e)) throw e;
  }
  const unmapped = present.filter((p) => !mapped.has(p.account));
  const accountLineFor = {}; // account -> the line label that claims it
  for (const l of lines) for (const a of l.accounts) accountLineFor[a] = l.label;
  return {
    lineLabels: lines.map((l) => l.label),
    mappedCount: mapped.size,
    present: present.map((p) => ({ ...p, line: accountLineFor[p.account] || null })),
    unmapped,
  };
}
