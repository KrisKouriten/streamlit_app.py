...
export const CLOSE_STEPS = [
  ...
  { code: "fa_playbook", stage: "WORKSTREAMS", kind: "AUTO", label: "Fixed-assets workstream complete", check: playbookCheck("FA") },
  {
    ...
    label: "Month-end close checklist complete",
    check: (s) => {
      const open = s.tasks?.open || 0;
      const total = s.tasks?.total || 0;
      const done = s.tasks?.done ?? (total - open);
      if (open === 0) {
        return { ok: true, detail: total ? `All ${total} entity close tasks complete across ${s.tasks?.entities || 0} entities` : "No month-end close tasks for the period" };
      }
      return { ok: false, detail: `${done}/${total} entity close tasks complete — ${open} outstanding` };
    },
  },
  {
