...
// A trade-pay reference as the close desk stores it: upper case, no spaces.
// Must match normTradePayRef in procurement-close-rules.js, which writes it.
export const compactRef = (s) => normRef(s).replace(/ /g, "");

/*
 * How a facility drawing ties to Procurement, or null when it should and does not.
 *
 *   { kind: "LC", dc_reference, purchase_ref }      an LC logged under the reference
 *   { kind: "TRADE_PAY", purchase_ref, supplier }   a paid purchase recording this
 *                                                   drawing as its WC… reference
 *   { kind: "NOT_PROCUREMENT", cost_driver }        Opex, Capex — not procurement
 *   null                                            procurement, but nothing logged
 *
 * Only LCs used to count, so every TradePay drawing read "Not in procurement"
 * even once Finance had recorded its WC reference on the purchase it paid.
 *
 * `row.proc_source` is the procurement source its cost driver maps to (the
 * caller sets it from facilitySourceOf; this file takes no imports). A drawing
 * with NO cost driver stays unmatched rather than being waved through as
 * not-procurement — blank is unknown, not Opex.
 */
export function procurementLink(row = {}, { lcByRef = new Map(), tradePayByRef = new Map() } = {}) {
  const lc = lcByRef.get(normRef(row.reference));
  if (lc) return { kind: "LC", ...lc };
  const tp = tradePayByRef.get(compactRef(row.reference));
  if (tp) return { kind: "TRADE_PAY", ...tp };
  const driver = String(row.cost_driver || "").trim();
  if (driver && row.proc_source == null) return { kind: "NOT_PROCUREMENT", cost_driver: driver };
  return null;
}
...
//
// A drawing is reconciled either way it can be: an LC logged under its
// reference (Miniso), or a paid purchase that records it as its trade-pay
// drawing (the WC… reference, migration 115). Drawings whose cost driver says
// they are not procurement at all (Opex, Capex) are never listed — there is
// nothing in Procurement they could be matched to.
export function unmatchedFacility(lcs = [], facilityRows = [], { tradePayByRef = new Map() } = {}) {
  const lcByRef = new Map();
  for (const l of lcs) { const n = normRef(l.lc_reference); if (isRealLcRef(n) && !lcByRef.has(n)) lcByRef.set(n, {}); }
  return facilityRows
    .filter((f) => procurementLink(f, { lcByRef, tradePayByRef }) == null)
...
const HEADER_ALIASES = {
  reference: "reference", referencenumber: "reference", bankreference: "reference", loanreference: "reference", ourreference: "reference", dealreference: "reference", drawingreference: "reference",
  ...
  loanamount: "loan_amount", loanamountinpaymentcurrency: "loan_amount", amount: "loan_amount", drawingamount: "loan_amount", principal: "loan_amount",
  ...
  loanperioddays: "loan_period_days", loanperiodindays: "loan_period_days", days: "loan_period_days", tenor: "loan_period_days", period: "loan_period_days", loanperiod: "loan_period_days",
  paymentamount: "payment_amount",
  paymentmonth: "payment_month", settlementmonth: "payment_month", month: "payment_month",
  // The sterling column. Worth being generous here: this is the figure Finance
  // put in the file precisely so nothing has to be converted, and a header this
  // list did not recognise was dropped in silence — which is how an upload that
  // DID carry GBP produced a register with an empty GBP column, and months
  // valued by conversion instead of by the number that was supplied.
  facilitypaymentgbp: "facility_payment_gbp", facilitypayment: "facility_payment_gbp", facilitygbp: "facility_payment_gbp",
  gbp: "facility_payment_gbp", gbpequivalent: "facility_payment_gbp", gbpamount: "facility_payment_gbp",
  gbpequivalentamount: "facility_payment_gbp", gbpvalue: "facility_payment_gbp", gbptotal: "facility_payment_gbp",
  amountgbp: "facility_payment_gbp", amountingbp: "facility_payment_gbp", valuegbp: "facility_payment_gbp",
  loanamountgbp: "facility_payment_gbp", loanamountingbp: "facility_payment_gbp",
  paymentamountgbp: "facility_payment_gbp", paymentamountingbp: "facility_payment_gbp",
  paymentgbp: "facility_payment_gbp", settlementgbp: "facility_payment_gbp", settlementamountgbp: "facility_payment_gbp",
  equivalentgbp: "facility_payment_gbp", equivalentingbp: "facility_payment_gbp", gbpeq: "facility_payment_gbp",
  sterling: "facility_payment_gbp", sterlingamount: "facility_payment_gbp", sterlingequivalent: "facility_payment_gbp",
  cashoutgbp: "facility_payment_gbp", outstandinggbp: "facility_payment_gbp",
  // Currency columns, which decide whether an amount gets converted at all.
  loanccy: "loan_currency", currencyofloan: "loan_currency", drawingcurrency: "loan_currency",
  paymentccy: "payment_currency", settlementcurrency: "payment_currency", settlementccy: "payment_currency",
};
...
/*
 * The payment amount, but only when it is STERLING.
 *
 * The extract states the drawing in its own currency (loan_amount) and the
 * settlement in sterling (payment_amount, with a £ in the cell) — while
 * payment_currency names the LOAN. So on a USD row, payment_amount IS pounds
 * despite the column next to it saying USD, and taking it as sterling is what
 * makes November read the £512,293 actually paid.
 *
 ...
 * 1.0000 is not an exchange rate; it means that cell was never converted, and
 * treating it as sterling values a €31,643 drawing at £31,643 instead of about
 * £27,516.
 *
 * payment_currency cannot tell these apart, because it says USD on the
 * converted rows too. What CAN is whether the figure was converted at all: a
 * payment materially different from the loan has had a rate applied; one equal
 * to it has not. Where it has not, nothing is returned and the caller converts
 * the loan amount properly.
 */
function sterlingPayment(rec) {
  const pay = csvNum(rec.payment_amount);
  if (pay == null) return null;
  const ccy = String(rec.loan_currency || rec.payment_currency || "GBP").toUpperCase();
  if (!ccy || ccy === "GBP") return pay;              // nothing to convert either way
  const loan = csvNum(rec.loan_amount);
  if (loan == null || !loan) return pay;              // no loan to compare against
  // Within a rounding hair of the loan → the same money, unconverted.
  const unconverted = Math.abs(pay - loan) <= Math.max(1, Math.abs(loan) * 0.001);
  return unconverted ? null : pay;
}
...
// Three-letter month → number, for bank exports that write dates as DD-Mon-YY.
const MONTH_ABBR = { jan: "01", feb: "02", mar: "03", apr: "04", may: "05", jun: "06", jul: "07", aug: "08", sep: "09", oct: "10", nov: "11", dec: "12" };

// Accept YYYY-MM-DD, DD/MM/YYYY (UK) or DD-Mon-YY / DD-Mon-YYYY (HSBC extract,
// e.g. 24-Aug-26); return 'YYYY-MM-DD' or null. `bad` set true on an unparseable
// non-blank value. A 2-digit year is read as 20YY.
function csvDate(v, bad) {
  ...
  m = /^(\d{1,2})[-/ ]([A-Za-z]{3})[A-Za-z]*[-/ ](\d{2}|\d{4})$/.exec(s);
  if (m) {
    const mo = MONTH_ABBR[m[2].toLowerCase()];
    if (mo) { const y = m[3].length === 2 ? `20${m[3]}` : m[3]; return `${y}-${mo}-${String(m[1]).padStart(2, "0")}`; }
  }
...
// Which columns a line maps to, for header detection.
const headerCols = (line) => splitCsvLine(line).map((h) => HEADER_ALIASES[normHeader(h)] || null);

// Find the header row. An HSBC extract often carries a title, a run date or a
// blank line above the column headers — the parser already skips the
// "***End***" footer below them, so the decoration at the top has to be handled
// too. Scores each line by how many columns it maps, requiring `reference`
// among them, and takes the best. A data row maps nothing, so it never wins.
// Returns -1 when no line looks like a header.
export function findFacilityHeaderRow(lines = []) {
  let best = -1, bestScore = 0;
  const limit = Math.min(lines.length, 30);
  for (let i = 0; i < limit; i++) {
    const cols = headerCols(lines[i]);
    if (!cols.includes("reference")) continue;
    const score = cols.filter(Boolean).length;
...
export function parseFacilityCsv(text) {
  const errors = [];
  // Blank lines are kept rather than filtered, so a reported row number is the
  // real line number in the file and stays useful when the header is offset.
  const lines = String(text || "").split(/\r\n|\r|\n/);
  if (lines.filter((l) => l.trim() !== "").length < 2) { errors.push("The file needs a header row and at least one data row."); return { rows: [], errors }; }
  const headerIdx = findFacilityHeaderRow(lines);
  if (headerIdx < 0) { errors.push("A 'reference' column is required (the bank drawing reference, e.g. LAIUK…)."); return { rows: [], errors }; }
  const cols = headerCols(lines[headerIdx]);
  /*
   * Columns this parser did not recognise, so the upload can SAY so.
   *
   * headerCols maps an unknown header to null and moves on. That is the right
   * behaviour — an extract carries plenty the register has no use for — but it
   * was doing it in silence, and a GBP column whose header was spelled a way
   * this list did not know was discarded with everything else. The register then
   * showed an empty GBP column, the desk converted at spot instead, and a month
   * read six figures light with nothing anywhere saying a column had been lost.
   *
   * Reported, not rejected: an unknown column is usually harmless, and a failed
   * upload would be worse than a noted one.
   */
  const ignored = splitCsvLine(lines[headerIdx])
    .map((h, i) => (cols[i] ? null : String(h || "").trim()))
    .filter((h) => h && h !== "");
  const rows = [];
  const seen = new Set();
  for (let i = headerIdx + 1; i < lines.length; i++) {
    ...
    if (!reference) { errors.push(`Line ${i + 1}: missing reference — skipped.`); continue; }
    // Skip the export's terminator/footer row, e.g. "************End************"
    // — decoration around the word "End", with no real drawing data. Silent (not
    // an error) since it is expected in the HSBC extract.
    if (/^end$/i.test(reference.replace(/[^a-z0-9]/gi, ""))) continue;
    const key = reference.toUpperCase();
    if (seen.has(key)) { errors.push(`Line ${i + 1}: duplicate reference “${reference}”.`); continue; }
    seen.add(key);
    const bad = {};
    const row = {
      reference,
      ...
      /*
       * THE STERLING CASH-OUT.
       *
       * The HSBC extract puts it in `payment_amount`, with a £ symbol in the
       * cell, while `payment_currency` names the LOAN's currency — USD on a
       * post-shipment buyer loan. Read literally that says "USD 171,259", and
       * the desk duly converted an already-sterling figure at spot: November
       * reported £385,183 against the £512,293 actually paid.
       *
       * So the payment amount IS the sterling settlement, and it fills this
       * column when the file does not give one of its own. csvNum strips the
       * symbol and the thousands separators either way.
       *
       * The loan pair stays untouched and keeps its own currency, which is what
       * makes the rate checkable: loan_amount / facility_payment_gbp is the rate
       * the bank actually dealt at, and on this extract it comes out at 1.3360
       * on every USD row — the hedged rate, not spot.
       */
      facility_payment_gbp: csvNum(rec.facility_payment_gbp) ?? sterlingPayment(rec),
      cost_driver: rec.cost_driver || null,
    };
    if (bad.v) errors.push(`Line ${i + 1} (“${reference}”): a date wasn't in DD/MM/YYYY or YYYY-MM-DD format.`);
    rows.push(row);
  }
