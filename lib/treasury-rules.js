...
// Is this LC visible on the bank trade facility yet? Matched on the customer
// reference first (the key the HSBC extract shares), then the LC/bank reference.
export function lcOnFacility(lc, facilityIndex) {
  for (const key of [lc.customer_reference, lc.lc_reference]) {
    const r = normRef(key);
