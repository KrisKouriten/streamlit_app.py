...
// Is this LC visible on the bank trade facility yet? Matched on the LC (bank)
// reference first — the reference Finance keys and HSBC draws under — then the
// customer reference as a fallback.
export function lcOnFacility(lc, facilityIndex) {
  for (const key of [lc.lc_reference, lc.customer_reference]) {
    const r = normRef(key);
    if (isRealLcRef(r)) { const m = facilityIndex.get(r); if (m) return m; }
  }
  return null;
}
// Does the LC's own USD value disagree with the facility drawing it matched?
const VALUE_TOL = 0.5;   // ignore sub-dollar rounding
export function lcValueMismatch(lc, match) {
  if (!match) return false;
  const a = lc.lc_amount, b = match.loan_amount;
  if (a == null || b == null) return false;
  return Math.abs(Number(a) - Number(b)) > VALUE_TOL;
}

// Facility drawings whose customer/bank reference matches no LC in Procurement —
// i.e. a bank drawing with nothing logged against it yet. Pure. Considers ALL
// LCs (not just DC-grouped ones).
export function unmatchedFacility(lcs = [], facilityRows = []) {
  const lcKeys = new Set();
  for (const l of lcs) for (const k of [l.customer_reference, l.lc_reference]) { const n = normRef(k); if (isRealLcRef(n)) lcKeys.add(n); }
  return facilityRows
    .filter((f) => ![normRef(f.customer_reference), normRef(f.reference)].some((k) => k && lcKeys.has(k)))
    .map((f) => ({ reference: f.reference, customer_reference: f.customer_reference || null,
      loan_amount: f.loan_amount == null ? null : Number(f.loan_amount), due_date: f.due_date || null, status: f.status || null }));
}
...
export function reconcileDcFacility(dcs = [], lcs = [], facilityRows = []) {
  ...
  for (const d of dcs) {
    groups.set(keyOf(d.purchase_id, d.dc_reference), {
      ...
      lcLogged: 0, facilityDrawn: 0, lcCount: 0, onFacilityCount: 0, notOnFacility: [], mismatch: [], lcs: [],
    });
  }
  for (const l of lcs) {
    ...
    const mismatch = lcValueMismatch(l, match);
    g.lcCount += 1;
    g.lcLogged = round2(g.lcLogged + amount);
    const entry = { lc_reference: l.lc_reference, customer_reference: l.customer_reference || null, lc_amount: amount, lc_settled: !!l.lc_settled,
      onFacility: !!match, valueMismatch: mismatch, facility_reference: match ? match.reference : null,
    ...
    g.lcs.push(entry);
    if (match) { g.onFacilityCount += 1; g.facilityDrawn = round2(g.facilityDrawn + num(match.loan_amount)); if (mismatch) g.mismatch.push(l.customer_reference || l.lc_reference || "(no reference)"); }
    else if (!l.lc_settled) g.notOnFacility.push(l.customer_reference || l.lc_reference || "(no reference)");
  }
  const finish = (g) => {
    ...
    g.mismatchCount = g.mismatch.length;
    return g;
  };
  ...
  const totals = {
    ...
    mismatchCount: out.reduce((t, g) => t + g.mismatchCount, 0),
  };
