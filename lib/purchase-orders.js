...
import {
  ...
  isInvoiceStatus, selfApprovalDecision, PO_APPROVAL_ROUTES,
  isoDay,
} from "./po-rules.js";
...
export async function createPo(input, actor) {
  ...
  // Entity to be invoiced (migration 107) — best-effort, degrades pre-107.
  if (input.invoice_entity_id) {
    try {
      await query(`UPDATE finance.purchase_order SET invoice_entity_id = $2 WHERE po_id = $1`,
        [poId, Number(input.invoice_entity_id)]);
    } catch (e) { if (e?.code !== "42703") throw e; }
  }
  // Description (migration 110) — best-effort, degrades pre-110.
  if (input.description != null && String(input.description).trim() !== "") {
    try {
      await query(`UPDATE finance.purchase_order SET description = $2 WHERE po_id = $1`,
        [poId, String(input.description).trim().slice(0, 300)]);
    } catch (e) { if (e?.code !== "42703") throw e; }
  }
...
export async function updatePo(poId, patch = {}, actor) {
  ...
  if ("invoice_entity_id" in patch) {
    try {
      await query(`UPDATE finance.purchase_order SET invoice_entity_id = $2 WHERE po_id = $1`,
        [poId, patch.invoice_entity_id ? Number(patch.invoice_entity_id) : null]);
    } catch (e) { if (e?.code !== "42703") throw e; }
  }
  if ("description" in patch) {
    try {
      await query(`UPDATE finance.purchase_order SET description = $2 WHERE po_id = $1`,
        [poId, patch.description ? String(patch.description).trim().slice(0, 300) : null]);
    } catch (e) { if (e?.code !== "42703") throw e; }
  }
...
export async function getPo(poId) {
  try {
    ...
    // Resolve the entity-to-be-invoiced name (migration 107). Skipped pre-107,
    // where invoice_entity_id is absent from the row (undefined).
    if (po.invoice_entity_id != null) {
      try {
        const { rows: er } = await query(`SELECT entity_name FROM core.dim_entity WHERE entity_id = $1`, [po.invoice_entity_id]);
        po.invoice_entity_name = er[0]?.entity_name || null;
      } catch { /* dim_entity absent — leave name unset */ }
    }
    const invoices = await listPoInvoices(poId);
    const auditTrail = await listPoAuditTrail(poId);
    return { po, recharge, invoices, auditTrail };
  } catch (e) {
...
// The full lifecycle history of a P.O — every audited event (raised, self /
// line-manager / finance approvals, challenges, reissues, closes and each update
// to the record), oldest first, for the P.O Summary & Close audit trail. Reads
// governance.audit_event; returns [] before governance is migrated so the detail
// panel degrades gracefully.
export async function listPoAuditTrail(poId) {
  try {
    const { rows } = await query(
      `SELECT occurred_at, actor_email, actor_name, event_type, detail
         FROM governance.audit_event
        WHERE object_type = 'purchase_order' AND object_ref = $1
        ORDER BY occurred_at, audit_id`, [String(poId)]);
    return rows;
  } catch (e) {
    if (tableMissing(e)) return [];
    throw e;
...
export async function listPoInvoices(poId) {
  try {
    // SELECT * so the per-invoice due_date (migration 109) comes through when
    // present and is simply absent before the column exists.
    const { rows } = await query(
      `SELECT * FROM finance.purchase_order_invoice WHERE po_id = $1 ORDER BY created_at, invoice_id`, [poId]);
    // Dates as 'YYYY-MM-DD' — the driver returns DATE columns as Date objects,
    // and everything downstream (the rollup, the sort, the UI) works on days.
    return rows.map((r) => ({
      ...r,
      invoice_amount: r.invoice_amount == null ? null : Number(r.invoice_amount),
      ...("due_date" in r ? { due_date: isoDay(r.due_date) } : {}),
      ...("paid_date" in r ? { paid_date: isoDay(r.paid_date) } : {}),
    }));
...
async function recomputePoInvoiceRollup(poId, actor) {
  ...
  // Roll up a representative invoice due date (migration 109) — the earliest set
  // due date across the P.O's invoices (the next payment deadline), or NULL when
  // none is set. Best-effort so the rollup still works before the column exists.
  const dueDates = invoices.map((i) => isoDay(i.due_date)).filter(Boolean).sort();
  try {
    await query(`UPDATE finance.purchase_order SET invoice_due_date = $2 WHERE po_id = $1`, [poId, dueDates[0] || null]);
  } catch (e) { if (e?.code !== "42703") throw e; }
  if (!invoices.length) {
    await query(
      `UPDATE finance.purchase_order
  ...
  const paidDates = invoices.filter((i) => i.paid && i.paid_date).map((i) => isoDay(i.paid_date)).filter(Boolean).sort();
...
export async function addPoInvoice(poId, input = {}, actor) {
  ...
  if (err) throw new Error(err);
  // Workflow status (migration 111) drives the paid flag: an invoice is paid iff
  // it's at the terminal "Paid" stage. Fall back to the legacy paid flag when no
  // status is supplied (older callers), and to Received otherwise.
  const status = isInvoiceStatus(input.invoice_status)
    ? input.invoice_status
    : (input.paid ? "PAID" : "RECEIVED");
  ...
  // Per-invoice workflow status (migration 111) — best-effort, degrades pre-111.
  try {
    await query(`UPDATE finance.purchase_order_invoice SET invoice_status = $2 WHERE invoice_id = $1`, [ins[0].invoice_id, status]);
  } catch (e) { if (e?.code !== "42703") throw e; }
  // Per-invoice due date (migration 109) — best-effort, degrades pre-109.
  if (input.due_date) {
    try {
      await query(`UPDATE finance.purchase_order_invoice SET due_date = $2 WHERE invoice_id = $1`, [ins[0].invoice_id, input.due_date]);
    } catch (e) { if (e?.code !== "42703") throw e; }
  }
...
export async function updatePoInvoice(invoiceId, patch = {}, actor) {
  ...
  // Workflow status (migration 111) is the primary control for paid-ness: moving
  // an invoice to "Paid" marks it paid, any earlier stage marks it unpaid. A bare
  // `paid` patch (legacy callers) is still honoured. `writeStatus` is written
  // best-effort after the main update, since the column may not exist pre-111.
  let writeStatus = null;
  if ("invoice_status" in patch && isInvoiceStatus(patch.invoice_status)) {
    writeStatus = patch.invoice_status;
    const paid = writeStatus === "PAID";
    sets.push(`paid = $${i++}`); vals.push(paid);
    sets.push(`paid_date = $${i++}`); vals.push(paid ? (patch.paid_date || new Date().toISOString().slice(0, 10)) : null);
  } else if ("paid" in patch) {
    ...
    writeStatus = patch.paid ? "PAID" : "RECEIVED";   // keep the workflow in step
  }
  if (sets.length) {
    vals.push(invoiceId);
  ...
  if (writeStatus) {
    try {
      await query(`UPDATE finance.purchase_order_invoice SET invoice_status = $2 WHERE invoice_id = $1`, [invoiceId, writeStatus]);
    } catch (e) { if (e?.code !== "42703") throw e; }
  }
  // Per-invoice due date (migration 109) — separate best-effort update so the
  // main update above stays safe before the column exists.
  if ("due_date" in patch) {
    try {
      await query(`UPDATE finance.purchase_order_invoice SET due_date = $2 WHERE invoice_id = $1`, [invoiceId, patch.due_date || null]);
    } catch (e) { if (e?.code !== "42703") throw e; }
  }
  if (!sets.length && !("due_date" in patch)) return { ok: true };
  const roll = await recomputePoInvoiceRollup(poId, actor);
...
export async function posForExport(ids = null) {
  ...
  // Resolve invoice-entity names (migration 107) in one lookup; degrades to no
  // names before the column / dimension exist.
  const entityIds = [...new Set(pos.map((p) => p.invoice_entity_id).filter((v) => v != null))];
  if (entityIds.length) {
    try {
      const { rows: er } = await query(`SELECT entity_id, entity_name FROM core.dim_entity WHERE entity_id = ANY($1::bigint[])`, [entityIds]);
      const names = new Map(er.map((e) => [String(e.entity_id), e.entity_name]));
      for (const p of byPo.values()) p.invoice_entity_name = p.invoice_entity_id != null ? (names.get(String(p.invoice_entity_id)) || null) : null;
    } catch { /* pre-migration — leave names unset */ }
  }
  return [...byPo.values()];
}

// When each P.O's invoice was last chased, from the audit log — { po_id: ISO }.
// No column of its own: the chase is an event, and the audit row already says
// who sent it, to whom, and when. Empty (never throws) if the log is unreadable.
export async function lastInvoiceChases() {
  try {
    const { rows } = await query(
      `SELECT object_ref, max(occurred_at) AS last_at, count(*)::int AS times
         FROM governance.audit_event
        WHERE event_type = 'po.invoice_chase' AND object_type = 'purchase_order'
        GROUP BY object_ref`);
    const out = {};
    for (const r of rows) out[r.object_ref] = { at: r.last_at instanceof Date ? r.last_at.toISOString() : String(r.last_at), times: r.times };
    return out;
  } catch { return {}; }
}
