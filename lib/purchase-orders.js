...
import {
  ...
  poTransitionError, isEditablePo,
} from "./po-rules.js";
...
const HEADER_FIELDS = {
  ...
  vat_amount: "vat_amount", po_category: "po_category", xero_po_number: "xero_po_number",
  fulfilment_date: "fulfilment_date", fulfilment_period: "fulfilment_period",
  department: "department", is_marketing: "is_marketing", marketing_levy: "marketing_levy",
  recharge_enabled: "recharge_enabled", notes: "notes",
};
...
export async function createPo(input, actor) {
  ...
  const { rows } = await query(
    `INSERT INTO finance.purchase_order
       (po_date, supplier, payment_terms, payment_date, currency, payment_value, vat_amount,
        po_category, xero_po_number, fulfilment_date, fulfilment_period, department,
        is_marketing, marketing_levy, recharge_enabled, invoice_action, notes, status, created_by)
    ...
    [input.po_date || null, input.supplier.trim(), input.payment_terms || null, input.payment_date || null,
     input.currency || "GBP", Number(input.payment_value), input.vat_amount != null && input.vat_amount !== "" ? Number(input.vat_amount) : null,
     input.po_category, input.xero_po_number.trim(), input.fulfilment_date || null, input.fulfilment_period || null,
     input.department.trim(), !!input.is_marketing, input.is_marketing ? (input.marketing_levy ?? null) : null,
     !!input.recharge_enabled, invoice_action, input.notes || null, actorOf(actor)]
  );
  const poId = rows[0].po_id;
  if (input.recharge_enabled && Array.isArray(input.recharge)) {
    await replaceRecharge(poId, input.recharge, Number(input.payment_value));
  }
...
export async function updatePo(poId, patch = {}, actor) {
  ...
  if ("recharge" in patch) {
    await replaceRecharge(poId, patch.recharge || [], Number(merged.payment_value));
  }
