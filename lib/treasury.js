...
import {
  ...
  isSalesStream, lcStage, lcStageLabel, facilityLifecycleSummary, reconcileDcFacility, parseFacilityCsv,
...
export async function getDcFacilityReconciliation() {
  return safe(async () => {
    ...
    const recon = reconcileDcFacility(
      ...
      lcs.map((l) => ({ ...l, lc_amount: l.lc_amount == null ? null : Number(l.lc_amount) })),
      facility);
    return { ready: true, ...recon };
  }, { ready: false, rows: [], totals: null });
}
