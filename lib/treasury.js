...
import {
  ...
  isSalesStream, lcStage, lcStageLabel, facilityLifecycleSummary, reconcileDcFacility, parseFacilityCsv, unmatchedFacility, normRef, compactRef, procurementLink,
} from "./treasury-rules.js";
import { facilityPosition } from "./suppliers.js";
import { facilityGbp, facilityImpliedRate, facilitySourceOf } from "./procurement-rules.js";
...
// The LC reference each Miniso DC groups its LCs under is the same reference HSBC
// draws under on the facility. Map facility drawing (LC) reference → the DC it was
// assigned to in Procurement, so each drawing shows its DC and drawings with no
// matching LC in Procurement can be flagged. Never throws (empty pre-migration).
async function lcDcMap() {
  try {
    const { rows } = await query(
      `SELECT l.lc_reference, l.dc_reference, p.reference AS purchase_ref
         FROM finance.procurement_lc l JOIN finance.procurement_purchase p ON p.purchase_id = l.purchase_id
        WHERE l.lc_reference IS NOT NULL AND btrim(l.lc_reference) <> ''`);
    const m = new Map();
    for (const r of rows) { const k = normRef(r.lc_reference); if (k && !m.has(k)) m.set(k, { dc_reference: r.dc_reference || null, purchase_ref: r.purchase_ref || null }); }
    return m;
  } catch { return new Map(); }
}

// Paid purchases that record which trade-pay drawing settled them (the WC…
// reference, migration 115), keyed the way the facility reference is compared.
// Empty before the migration, so Treasury still renders.
async function tradePayRefMap() {
  try {
    const { rows } = await query(
      `SELECT trade_pay_ref, reference AS purchase_ref, supplier
         FROM finance.procurement_purchase
        WHERE payment_method = 'TRADE_PAY' AND trade_pay_ref IS NOT NULL AND btrim(trade_pay_ref) <> ''`);
    const m = new Map();
    for (const r of rows) { const k = compactRef(r.trade_pay_ref); if (k && !m.has(k)) m.set(k, { purchase_ref: r.purchase_ref || null, supplier: r.supplier || null }); }
    return m;
  } catch { return new Map(); }
}
...
export async function getTradeFacility({ costDriver = null, product = null } = {}) {
  return safe(async () => {
    ...
    const [dcByLc, tradePayByRef] = await Promise.all([lcDcMap(), tradePayRefMap()]);
    // The GBP this drawing is COUNTED as, which is not always the GBP on the
    // extract. Most HSBC rows carry no GBP figure at all, so Trade-pay spend
    // converts the payment amount at spot — and until now that figure existed
    // nowhere on screen. The register showed a USD amount and an empty GBP
    // column, and a month's drawings were added up in the wrong currency.
    const fxRates = await getFxRates().catch(() => []);
    const rateFor = (ccy) => findRate(fxRates, ccy, "SPOT");
    const rows = raw.map((r) => {
      const link = procurementLink({ ...r, proc_source: facilitySourceOf(r) }, { lcByRef: dcByLc, tradePayByRef });
      const hit = link?.kind === "LC" ? link : null;
      return {
        ...r,
        // Exactly what tradeSpendByMonth would value this row at.
        gbp_at_spot: facilityGbp(r, rateFor),
        // The rate the bank dealt at, from the two figures it supplied. Shown so
        // the hedge is checkable per drawing instead of taken on trust.
        implied_rate: facilityImpliedRate(r),
        loan_start_date: isoDate(r.loan_start_date),
        due_date: isoDate(r.due_date),
        payment_month: isoDate(r.payment_month),
        // DC this drawing's LC is assigned to in Procurement (null = no DC set / not logged).
        dc_reference: hit ? hit.dc_reference : null,
        dc_purchase_ref: hit ? hit.purchase_ref : null,
        // How it ties to Procurement: LC, TRADE_PAY (a paid purchase records
        // this WC reference), NOT_PROCUREMENT (Opex / Capex), or null.
        proc_link: link ? link.kind : null,
        proc_purchase_ref: link?.kind === "TRADE_PAY" ? link.purchase_ref : null,
        proc_supplier: link?.kind === "TRADE_PAY" ? link.supplier : null,
        in_procurement: link?.kind === "LC" || link?.kind === "TRADE_PAY",
      };
    });
...
export async function uploadTradeFacility(csv, actor) {
  ...
  await audit({ actor, eventType: "treasury.facility.upload", objectType: "bank_trade_facility", objectRef: "replace", detail: { rows: rows.length, ignored } });
  // `ignored` is handed back so the desk can name the columns that were not
  // read. A dropped GBP column cost real money to find; it should never again
  // be something you can only discover by adding a month up by hand.
  return { ok: true, loaded: rows.length, ignored };
}
...
export async function getDcFacilityReconciliation() {
  return safe(async () => {
    const [{ rows: dcs }, { rows: lcs }, { rows: facility }] = await Promise.all([
      ...
      query(`SELECT l.purchase_id, l.dc_reference, l.lc_reference, l.lc_amount, l.lc_settled, COALESCE(p.currency,'USD') AS currency
               FROM finance.procurement_lc l JOIN finance.procurement_purchase p ON p.purchase_id = l.purchase_id`),
      query(`SELECT reference, beneficiary, customer_reference, loan_amount, loan_currency, outstanding_amount,
                    to_char(due_date,'YYYY-MM-DD') AS due_date, status, cost_driver
               FROM finance.bank_trade_facility`),
    ]);
    const tradePayByRef = await tradePayRefMap();
    ...
    const orphans = unmatchedFacility(lcClean, facility.map((f) => ({ ...f, proc_source: facilitySourceOf(f) })), { tradePayByRef });
