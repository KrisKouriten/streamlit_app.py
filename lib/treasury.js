...
import {
  ...
  isSalesStream, lcStage, lcStageLabel, facilityLifecycleSummary, reconcileDcFacility, parseFacilityCsv, unmatchedFacility, normRef,
} from "./treasury-rules.js";
import { facilityPosition } from "./suppliers.js";
import { facilityGbp, facilityImpliedRate } from "./procurement-rules.js";
...
export async function getTradeFacility({ costDriver = null, product = null } = {}) {
  return safe(async () => {
    ...
    const rows = raw.map((r) => {
      const hit = dcByLc.get(normRef(r.reference));
      return {
        ...r,
        ...
        in_procurement: !!hit,   // false → LC on the facility but not logged in Procurement
      };
    });
