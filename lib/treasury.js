...
// Facility (bank) reference → customer reference pairs, so the LC form can
// auto-fill the customer reference from the LC reference the user keys (matched
// to the HSBC drawing). Never throws; empty before the facility table/data exist.
export async function facilityRefPairs() {
  try {
    const { rows } = await query(
      `SELECT DISTINCT btrim(reference) AS reference, btrim(customer_reference) AS customer_reference
         FROM finance.bank_trade_facility
        WHERE reference IS NOT NULL AND btrim(reference) <> ''
          AND customer_reference IS NOT NULL AND btrim(customer_reference) <> ''`);
    return rows.map((r) => ({ reference: r.reference, customer_reference: r.customer_reference }));
  } catch { return []; }
}
