...
const PUBLIC_EXACT = new Set([
  "/login",
  "/set-password", // invited/reset users have no session yet; the one-time token is the auth
  "/api/auth/login",
  "/api/auth/mfa-verify", // second factor: authed by the pending ticket, not a session
  "/api/auth/set-password", // self-guarding: the one-time invite token is the authorisation
  "/api/auth/logout",
  "/api/auth/me",
  "/api/health",
  "/api/joiin-refresh",
...
// Paths a user with a pending forced password-change may still reach — the
// change screen itself, the endpoint that sets the new password, and the
// escape hatches (sign out, identity probe). Everything else is blocked until
// they have changed their starter password.
...
/*
 * Should this request be diverted to the forced password-change screen?
 * True when the session carries the mustChange flag and the path is neither a
 * public route nor one of the change-password essentials. Pure + edge-safe so
 * the middleware (Edge) can call it straight from the JWT claim.
 */
export function mustChangePasswordGate(pathname, mustChange) {
...
/*
 * CSRF guard: for a state-changing request, is the browser Origin same-origin
 * with the request Host? A cross-site page can make the browser POST with our
 * cookies, but it cannot forge the Origin header. Non-browser clients (the
 * Vercel cron, server-to-server calls) send no Origin — allowed, because they
 * carry their own auth (bearer token) and CSRF is a browser-only threat.
 * A present-but-mismatched or malformed Origin is refused.
 */
