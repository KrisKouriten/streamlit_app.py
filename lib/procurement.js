...
// + how a paid row settled (migration 113) — what the cash side of "spent" reads.
const ORDER_COLS_PM = `${ORDER_COLS}, payment_status, paid_date, payment_method`;
...
async function fetchOrders() {
  try {
    const { rows } = await query(`SELECT ${ORDER_COLS_PM} FROM finance.procurement_purchase ORDER BY created_at DESC, purchase_id DESC`);
    return rows;
  } catch (e) {
    if (!columnMissing(e)) throw e;              // pre-113: no payment_method
  }
  try {
