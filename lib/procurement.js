  ...
  try {
    const { rows } = await query(
      `SELECT cost_driver, to_char(due_date,'YYYY-MM') AS due_date,
              to_char(payment_month,'YYYY-MM') AS payment_month, facility_payment_gbp
...
export async function getProcurement() {
  try {
    const [orders, { rows: budgets }, facility] = await Promise.all([
      fetchOrders(),
    ...
    const spend = { trade: tradeSpendByMonth(facility), cash: cashSpendByMonth(active) };
