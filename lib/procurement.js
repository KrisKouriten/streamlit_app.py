...
async function fetchOrders() {
  try {
    ...
    if (columnMissing(e)) {
      const { rows } = await query(`SELECT purchase_id, source, supplier, category, order_ym, amount_gbp, terms_days, status, reference, source_tag, pickup_date, delivery_ym, 'APPROVED' AS approval_status, created_by, created_at FROM finance.procurement_purchase ORDER BY created_at DESC, purchase_id DESC`);
      return rows;
    }
    throw e;
  }
