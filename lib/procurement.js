...
export async function addProcurementPurchase({ source, supplier, category, order_ym, amount_gbp, terms_days, status, reference }, actor) {
  ...
  await query(
    ...
    [source, supplier.trim(), (category || "").trim() || null, order_ym, Number(amount_gbp), Number(terms_days) || 0,
     status === "PAID" ? "PAID" : "COMMITTED", (reference || "").trim() || null, actor]
  );
  await audit({ actor, eventType: "procurement.purchase.add", objectType: "procurement_purchase", objectRef: `${source}·${supplier}`, detail: { amount_gbp: Number(amount_gbp), order_ym } });
  return { ok: true };
}
