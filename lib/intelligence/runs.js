...
export async function openRun({ surface, session, pageId = null, questionType = null, question = null, filters = null, model = null, promptCode = null, scopeNote = null }) {
  try {
    const { rows } = await query(
      `INSERT INTO intelligence.ai_run
         (surface, user_id, user_email, page_id, question_type, question, filters, model, prompt_code, scope_note, status)
      ...
      [surface, session?.id ?? null, session?.email ?? null, pageId, questionType,
       ...
       filters ? JSON.stringify(filters) : null, model, promptCode, scopeNote]
    );
