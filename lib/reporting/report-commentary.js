...
import { buildReportContext, renderContextPreamble, PERSPECTIVES, isPerspective, resolveAudience, audienceBanner } from "./commentary-perspectives";
 ...
 */
export async function generateReportCommentary({ reportId, sectionInstId, componentId = null, perspective = "EXECUTIVE", detailLevel = "STANDARD", tone = "MANAGEMENT", audience = null, actor } = {}) {
  // An audience register (e.g. Board, Investor) resolves to a perspective + tone +
  // extra focus, and — when sensitive — a governance banner stamped on the draft.
  let aud = null, extraFocus = [];
  if (audience) {
    aud = resolveAudience(audience);
    if (!aud) return { ok: false, error: `Unknown audience: ${audience}` };
    perspective = aud.perspective;
    tone = aud.tone;
    extraFocus = aud.focus || [];
  }
  ...
  const ctx = buildReportContext({
    ...
    dataThroughDate: report.data_through_date, audience: aud ? aud.label : report.audience,
  ...
  try {
    ...
    const out = await generateGoverned({
      ...
      user: buildUser(ctx, [...PERSPECTIVES[perspective].focus, ...extraFocus], envelope),
    ...
    // Sensitive audiences (Board / Investor / Bank / CEO) get a governance banner
    // stamped on the draft so the listing-rules caveat travels into every export.
    const banner = audienceBanner(aud);
    const bodyText = [banner, banner ? "" : null, draft.headline, "", draft.body].filter((x) => x != null && x !== undefined).join("\n");
    const structured = {
    ...
    if (compId) {
      ...
      const { rows } = await query(
        ...
        [reportId, sectionInstId, `${aud ? aud.label : PERSPECTIVES[perspective].label} commentary`, pos[0].n, perspective,
