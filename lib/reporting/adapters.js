...
import { procRef, lineValue, displayStatus } from "../procurement-close-rules.js";
...
export const SOURCE_ADAPTERS = {
  ...
  // Procurement — merchandising supplier spend and its Finance close status.
  procurement: async ({ filters }) => {
    const env = envelope("procurement", "Procurement", "/operate/procurement-summary", { filters });
    try {
      const r = await procurementRollup();
      if (!r || !r.ready || !r.count) { env.reason = "No procurement purchases recorded."; env.metadata.validationStatus = "MISSING"; return env; }
      env.kpis = [
        { label: "Committed spend", value: r.committed, unit: "£" },
        { label: "Open (pending + approved)", value: r.open, unit: "£" },
        { label: "Under challenge", value: r.challengedValue, unit: "£" },
        { label: "Cash budget", value: r.cashBudget, unit: "£" },
        { label: "Purchases", value: r.count, unit: "count" },
        { label: "Closed", value: r.committedCount, unit: "count" },
      ].filter((k) => k.value != null);
      env.table = {
        columns: [
          ...
          { key: "value", label: "Net", align: "right", money: true },
          { key: "status", label: "Status" },
        ],
        rows: (r.register || []).slice(0, 15).map((p) => ({
          reference: procRef(p), source: p.source, supplier: p.supplier, channel: p.channel_code || "—",
          value: lineValue(p), status: displayStatus(p).label,
        })),
      };
      env.metadata.dataThrough = `${r.count} purchases · ${r.committedCount} closed`;
      env.ready = true;
    } catch (e) { env.reason = `Procurement unavailable: ${e.message}`; env.metadata.validationStatus = "MISSING"; }
    return env;
  },
