...
export async function createReport(input, actor) {
  ...
  // The report's scope. For the Departmental Budget Pack a budget holder chooses
  // which departmental budget the pack is built from; carry that on the scope so
  // the report records what it was scoped to, and (below) stamp it onto the
  // dept_budget-sourced sections' filters so the adapter resolves that budget.
  const scope = input.scope && typeof input.scope === "object" ? { ...input.scope } : {};
  const department = input.department ? String(input.department).trim() : null;
  const budgetId = input.budgetId ? Number(input.budgetId) : null;
  if (department) scope.department = department;
  if (budgetId) scope.budgetId = budgetId;
  ...
  // Departmental Budget Pack: point every dept_budget-sourced section at the
  // chosen departmental budget (else the adapter falls back to the latest budget
  // across all departments). Stored on section filters so a report scoped to
  // "Marketing" resolves Marketing's budget on every page.
  if (department || budgetId) {
    const secFilters = {};
    if (department) secFilters.department = department;
    if (budgetId) secFilters.budgetId = budgetId;
    await query(
      `UPDATE finance.report_section_instance
          SET filters = COALESCE(filters, '{}'::jsonb) || $2::jsonb
        WHERE report_id = $1 AND source_key = 'dept_budget'`,
      [reportId, JSON.stringify(secFilters)]
    );
  }

  await audit({ actor, eventType: "report.create", objectType: "report_instance", objectRef: String(reportId), detail: { template: tpl.template_key, title, department: department || null, budgetId: budgetId || null } });
...
// Reads
// ---------------------------------------------------------------------------
export async function listReports({ owner = null, status = null, templateKey = null, limit = 50 } = {}) {
  try {
    const { rows } = await query(
      `SELECT report_id, template_key, title, reporting_period, data_through_date, status,
       ...
       WHERE ($1::varchar IS NULL OR owner = $1)
         ...
         AND ($3::varchar IS NULL OR template_key = $3)
       ORDER BY updated_at DESC LIMIT $4`,
      [owner, status, templateKey, limit]
    );
