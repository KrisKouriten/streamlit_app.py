...
export async function procurementRollup() {
  try {
    ...
    // full documentary-credit amount keyed on the desk; the total LC value is the
    // sum of the LC amounts logged under it (a DC can hold several); the balance is
    // DC value − total LC value. GBP columns convert the LC total at the spot and
    // costing FX rates, with the earliest expected LC payment date.
    ...
    if (minisoIds.length) {
      ...
      // Aggregate the LCs under each DC: count, USD total, earliest expected payment.
      const byDc = new Map();
      for (const l of lcRecs) {
        const k = `${l.purchase_id}·${normRef(l.dc_reference || "")}`;
        const cur = byDc.get(k) || { count: 0, usd: 0, due: null };
        cur.count += 1;
      ...
      const dcRows = dcRecs.map((d) => {
        const agg = byDc.get(`${d.purchase_id}·${normRef(d.dc_reference || "")}`) || { count: 0, usd: 0, due: null };
        const face = d.dc_value == null ? null : Number(d.dc_value);
        const lcTotal = agg.usd;
        return {
          dc_reference: d.dc_reference, purchaseRef: refByPid[d.purchase_id] || `#${d.purchase_id}`,
          count: agg.count,
          dc_value: face, lc_value: lcTotal,
          balance: face == null ? null : round2(face - lcTotal),
          gbp_spot: convertToGbp(lcTotal, spotRate), gbp_costing: convertToGbp(lcTotal, costRate),
          due_date: agg.due,
        };
      });
      const totalFace = round2(dcRows.reduce((t, r) => t + (r.dc_value || 0), 0));
      const totalLc = round2(dcRows.reduce((t, r) => t + (r.lc_value || 0), 0));
      dc = {
        count: dcRows.length, currency: "USD",
        totalValue: totalFace, totalLcValue: totalLc, totalBalance: round2(totalFace - totalLc),
        totalSpotGbp: convertToGbp(totalLc, spotRate) || 0,
        totalCostingGbp: convertToGbp(totalLc, costRate) || 0,
        rows: dcRows.sort((a, b) => (b.lc_value || 0) - (a.lc_value || 0)).slice(0, 50),
      };
    }
