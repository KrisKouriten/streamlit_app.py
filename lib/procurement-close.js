...
import { reconcileDcFacility } from "./treasury-rules.js";
...
//   l.on_facility           true / false / null (null = facility unreadable or
//                            no real reference yet) — matched on customer_reference
//                            first (the HSBC-shared key), then the LC/bank reference.
//   l.facility_value_mismatch  true when the LC's own USD value ≠ the matched
//                            drawing's loan_amount (beyond a sub-dollar tolerance).
...
async function attachOnFacility(lcs) {
  ...
  let byRef = null;
  try {
    const { rows } = await query(`SELECT reference, customer_reference, loan_amount FROM finance.bank_trade_facility`);
    byRef = new Map();
    for (const f of rows) { for (const k of [f.customer_reference, f.reference]) { const n = normRef(k); if (n && !byRef.has(n)) byRef.set(n, f); } }
  } catch (e) { if (!absent(e)) throw e; byRef = null; }   // facility table absent → unknown
  for (const l of lcs) {
    if (byRef == null) { l.on_facility = null; l.facility_value_mismatch = null; continue; }
    let match = null;
    for (const k of [l.customer_reference, l.lc_reference]) { const n = normRef(k); if (!NON_REF.has(n)) { const m = byRef.get(n); if (m) { match = m; break; } } }
    // A real reference that isn't found → false; a placeholder-only LC → null (no nag).
    const hasRealRef = [l.customer_reference, l.lc_reference].some((k) => !NON_REF.has(normRef(k)));
    l.on_facility = match ? true : (hasRealRef ? false : null);
    l.facility_value_mismatch = match && match.loan_amount != null && l.lc_amount != null
      ? Math.abs(Number(l.lc_amount) - Number(match.loan_amount)) > 0.5 : false;
  }
}
...
const LC_COLS = `lc_id, purchase_id, dc_reference, customer_reference, lc_reference, lc_amount, lc_bank,
...
// Pre-094 fallback (customer_reference absent) — everything else present.
const LC_COLS_094 = LC_COLS.replace("dc_reference, customer_reference,", "dc_reference, NULL::varchar AS customer_reference,");
// Pre-084 fallback columns (DC ref / customer ref / loan / arrival / actual-payment absent).
const LC_COLS_LEGACY = `lc_id, purchase_id, NULL::varchar AS dc_reference, NULL::varchar AS customer_reference, lc_reference, lc_amount, lc_bank,
  to_char(lc_confirmed_date,'YYYY-MM-DD') AS lc_confirmed_date,
  to_char(lc_payment_date,'YYYY-MM-DD') AS lc_payment_date,
...
// Select LCs with a 3-tier column fallback (094 → 084 → legacy) so the desk keeps
// working before each migration is applied. Returns [] if the table is absent.
async function selectLcs(where, params) {
  for (const cols of [LC_COLS, LC_COLS_094, LC_COLS_LEGACY]) {
...
export async function updateLc(lcId, fields = {}, actor) {
  ...
  if ("customer_reference" in fields) put("customer_reference", (fields.customer_reference || "").trim() || null);
...
export async function addLc(purchaseId, { dc_reference, customer_reference, lc_reference, lc_amount, lc_bank, lc_confirmed_date, lc_payment_date }, actor) {
  ...
  const cust = (customer_reference || "").trim() || null;
  const ref = String(lc_reference).trim();
  const bank = lc_bank || LC_BANK_DEFAULT;
  // Tiered insert so it works before migrations 094 / 084 are applied.
  const inserts = [
    { cols: "purchase_id, dc_reference, customer_reference, lc_reference, lc_amount, lc_bank, lc_confirmed_date, lc_payment_date, created_by",
      vals: [purchaseId, dc, cust, ref, numOrNull(lc_amount), bank, lc_confirmed_date || null, lc_payment_date || null, actorOf(actor)] },
...
export async function procurementRollup() {
  try {
    ...
    // ---- Documentary Credit drawdown vs bank trade facility (Miniso HQ) ----
    // Consolidate every DC across the merch-active Miniso requests: value vs LCs
    // logged vs what HSBC has actually drawn on the facility → balance remaining,
    // and how many LCs aren't on the facility yet. Same reconciliation the
    // Treasury desk shows. Denominated in the DC currency (USD).
    const minisoIds = rows.filter((r) => r.source === "MINISO").map((r) => r.purchase_id);
    let dc = { count: 0, currency: "USD", totalValue: 0, totalUsed: 0, totalDrawn: 0, totalRemaining: 0, notOnFacilityCount: 0, overCount: 0, rows: [] };
    if (minisoIds.length) {
      const [dcRecs, lcRecs, facRecs] = await Promise.all([
        ...
        query(`SELECT reference, customer_reference, loan_amount, outstanding_amount, to_char(due_date,'YYYY-MM-DD') AS due_date, status FROM finance.bank_trade_facility`).then((r) => r.rows).catch(() => []),
      ]);
      const refByPid = {}, ccyByPid = {};
      for (const r of rows) { refByPid[r.purchase_id] = r.reference || `#${r.purchase_id}`; ccyByPid[r.purchase_id] = r.currency || "USD"; }
      const dcsForRecon = dcRecs.map((d) => ({ dc_id: d.dc_id, purchase_id: d.purchase_id, dc_reference: d.dc_reference, dc_value: d.dc_value == null ? null : Number(d.dc_value), currency: d.currency || ccyByPid[d.purchase_id] || "USD", purchase_ref: refByPid[d.purchase_id] }));
      const lcsForRecon = lcRecs.map((l) => ({ purchase_id: l.purchase_id, dc_reference: l.dc_reference, customer_reference: l.customer_reference, lc_reference: l.lc_reference, lc_amount: l.lc_amount == null ? null : Number(l.lc_amount), lc_settled: l.lc_settled }));
      const { rows: recon, totals } = reconcileDcFacility(dcsForRecon, lcsForRecon, facRecs);
      dc = {
        count: recon.length,
        currency: totals.currency,
        totalValue: totals.totalValue,
        totalUsed: totals.totalLogged,        // LCs logged
        totalDrawn: totals.totalDrawn,        // drawn on the HSBC facility
        totalRemaining: totals.totalRemaining, // value − logged
        notOnFacilityCount: totals.notOnFacilityCount,
        overCount: recon.filter((g) => g.over).length,
        rows: recon.sort((a, b) => (b.lcLogged || 0) - (a.lcLogged || 0)).slice(0, 50).map((g) => ({
          dc_reference: g.dc_reference, purchaseRef: g.purchase_ref, currency: g.currency, count: g.lcCount,
          dc_value: g.dc_value, used: g.lcLogged, drawn: g.facilityDrawn, remaining: g.remaining,
          notOnFacility: g.notOnFacilityCount, over: g.over,
        })),
      };
    }
