...
export async function listForClose({ source = null, financeStatus = null, limit = 500 } = {}) {
  ...
  try {
    ...
    try {
      ...
      if (!columnMissing(e)) throw e;             // pre-085: no FX columns
...
const LC_COLS = `lc_id, purchase_id, dc_reference, lc_reference, lc_amount, lc_bank, lc_confirmed_date, lc_payment_date, lc_settled, lc_settled_date, lc_settled_amount, loan_type, goods_arrived_date, actual_payment_date, created_by, created_at`;
// Pre-084 fallback columns (DC ref / loan / arrival / actual-payment absent).
const LC_COLS_LEGACY = `lc_id, purchase_id, NULL::varchar AS dc_reference, lc_reference, lc_amount, lc_bank, lc_confirmed_date, lc_payment_date, lc_settled, lc_settled_date, lc_settled_amount, 'IMPORT' AS loan_type, NULL::date AS goods_arrived_date, NULL::date AS actual_payment_date, created_by, created_at`;
