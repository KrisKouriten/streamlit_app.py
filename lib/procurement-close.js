...
// Pickup date (migration 076) and approval status (082). The close desk shows
// neither, but both decide which month a row belongs to and whether it counts at
// all: a Miniso order's 180-day clock starts at pickup, so without pickup_date
// `cashOutFor` silently falls back to order month + terms and puts the row in a
// different month than the budget tables do — the two screens then disagree
// about the same order. Cancelled orders must not count against a budget either.
const COLS_PU = `${COLS}, pickup_date, approval_status`;
...
const COLS_FX = `${COLS_PU}, currency, amount_ccy, cost_rate_type, cost_fx_rate, stock_rate_type, stock_fx_rate, stock_value_gbp`;
const COLS_FXR = `${COLS_FX}, report_rate_type`; // + FX reporting basis (migration 091)
const COLS_PM = `${COLS_FXR}, payment_method`;   // + how a paid row settled (migration 113)
...
export async function listForClose({ source = null, financeStatus = null, limit = 500 } = {}) {
  ...
  try {
    let rows;
    try {
      ({ rows } = await query(`SELECT ${COLS_PM} FROM finance.procurement_purchase ${where}`, params));
    } catch (e0) {
      if (!columnMissing(e0)) throw e0;           // pre-113: no payment_method
      try {
        ...
        try {
          ...
          try {
            ({ rows } = await query(`SELECT ${COLS_PU} FROM finance.procurement_purchase ${where}`, params));
          } catch (e3) {
            if (!columnMissing(e3)) throw e3;     // pre-076/082: no pickup date or approval status
