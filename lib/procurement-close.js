...
/*
 * Optional columns, each added by a later migration, each independent of the
 * others. Ask the catalogue which ones exist and select exactly those.
 *
 * This replaces a nested try/catch ladder that tried progressively shorter
 * column lists. The ladder assumed the migrations arrive in order, so each tier
 * was built on the one below it — and that made every richer tier depend on
 * every poorer one. When `approval_status` turned out to be missing on the live
 * database, the FX tier was built on top of it, so the whole chain fell through
 * to the base columns and the desk silently lost the FX figures it had always
 * had. One absent column cost four unrelated features.
 *
 * A catalogue lookup cannot degrade that way: a missing column costs only
 * itself. It also self-heals — apply the migration and the column appears on the
 * next read, with no deploy.
 */
const OPTIONAL_COLS = [
...
// Which of the optional columns this database actually has. One cheap catalogue
// read per call — no cache, so applying a migration takes effect immediately
// rather than at the next cold start.
async function presentCols() {
  try {
    const { rows } = await query(
      `SELECT column_name FROM information_schema.columns
        WHERE table_schema = 'finance' AND table_name = 'procurement_purchase'
          AND column_name = ANY($1)`, [OPTIONAL_COLS]);
    return rows.map((r) => r.column_name);
  } catch {
    return [];   // catalogue unreadable — fall back to the columns that always exist
  }
}

async function selectCols() {
  const extra = await presentCols();
  return extra.length ? `${COLS}, ${extra.join(", ")}` : COLS;
}
