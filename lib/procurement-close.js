  ...
  try {
    ...
    if (minisoIds.length) {
      const [dcRecs, lcRecs, facRecs] = await Promise.all([
        ...
        query(`SELECT purchase_id, dc_reference, lc_reference, lc_amount, lc_settled FROM finance.procurement_lc WHERE purchase_id = ANY($1)`, [minisoIds]).then((r) => r.rows).catch(() => []),
