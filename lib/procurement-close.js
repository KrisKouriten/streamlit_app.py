  ...
  try {
    ...
    if (minisoIds.length) {
      const [dcRecs, lcRecs, facRecs] = await Promise.all([
        ...
        query(`SELECT reference, loan_amount, outstanding_amount, to_char(due_date,'YYYY-MM-DD') AS due_date, status FROM finance.bank_trade_facility`).then((r) => r.rows).catch(() => []),
      ]);
      ...
      const lcsForRecon = lcRecs.map((l) => ({ purchase_id: l.purchase_id, dc_reference: l.dc_reference, lc_reference: l.lc_reference, lc_amount: l.lc_amount == null ? null : Number(l.lc_amount), lc_settled: l.lc_settled }));
