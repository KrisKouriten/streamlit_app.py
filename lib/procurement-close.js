...
import { financeActionError, isProcChallengeReason, challengeNoteError, isProcPaymentStatus, isProcPaymentMethod, committedAmount, lineValue, lcActionError, LC_BANK_DEFAULT, reportBasis, reportedGbp, REPORT_RATE_TYPES, dcDrawdown, validateDc, normTradePayRef, tradePayRefError, tradePayMatch } from "./procurement-close-rules.js";
import { getFxRates } from "./fx";
import { findRate, convertToGbp } from "./fx-rules.js";
import { supplierMetaByNorm } from "./suppliers";
import { facilityRefIndex, lcOnFacility } from "./treasury-rules.js";
...
/*
 * Optional columns on finance.procurement_purchase, each added by a later
 * migration and each independent of the others. `selectList` asks the catalogue
 * which exist and selects exactly those, so a missing column costs only itself.
 * See lib/db-columns.js for why the try/catch ladder this replaced was wrong.
 */
const OPTIONAL_COLS = [
  "pickup_date",       // 076 — Miniso's 180-day clock starts here, so it decides the cash-out month
  "delivery_ym",       // 076
  "approval_status",   // 082 — raise → head of department → Finance; cancelled must not count
  "currency",          // 085 — order currency and the FX basis below
  "amount_ccy", "cost_rate_type", "cost_fx_rate", "stock_rate_type", "stock_fx_rate", "stock_value_gbp",
  "report_rate_type",  // 091
  "payment_method",    // 113 — cash or trade pay, once paid
  "trade_pay_ref",     // 115 — WHICH drawing a trade-pay row settled on
  "vat_rate",          // 116 — amount_gbp is NET; the gross is what leaves the bank
];

const selectCols = () => selectList("finance", "procurement_purchase", COLS, OPTIONAL_COLS);
...
export async function listForClose({ source = null, financeStatus = null, limit = 500 } = {}) {
  ...
  try {
    ...
    const { rows } = await query(`SELECT ${cols} FROM finance.procurement_purchase ${where}`, params);
    attachReportGbp(rows, await getFxRates().catch(() => []));
    await attachLcs(rows);
    await attachTradePayMatch(rows);
...
/*
 * Reconcile each trade-pay row against the HSBC facility (migration 115).
 *
 * The same loose match the LCs already get: the reference is looked up in
 * finance.bank_trade_facility, and NOT FOUND is a flag, not an error. The
 * extract is uploaded periodically, so a genuine reference may simply not be
 * loaded yet.
 *
 * `settled` carries what the facility says about that drawing, because the point
 * of the link is to close a procurement order once the loan behind it is paid
 * off: outstanding_amount at zero is the signal.
 */
async function attachTradePayMatch(rows) {
  const wanted = rows.filter((r) => r.payment_method === "TRADE_PAY" && normTradePayRef(r.trade_pay_ref));
  if (!wanted.length) return;
  let byRef = null;
  try {
    const { rows: f } = await query(
      `SELECT reference, outstanding_amount, facility_payment_gbp, due_date, status FROM finance.bank_trade_facility`);
    byRef = new Map();
    for (const d of f) { const n = normTradePayRef(d.reference); if (n && !byRef.has(n)) byRef.set(n, d); }
  } catch (e) { if (!absent(e)) throw e; byRef = null; }   // facility absent → unknown, not unmatched
  const refs = byRef ? new Set(byRef.keys()) : null;
  for (const r of rows) {
    r.trade_pay = tradePayMatch(r, refs);
    const hit = byRef && r.trade_pay.ref ? byRef.get(r.trade_pay.ref) : null;
    // Outstanding nil means the facility has been repaid, which is what makes
    // the procurement order closable. Null where the facility cannot say.
    r.trade_pay_outstanding = hit && hit.outstanding_amount != null ? Number(hit.outstanding_amount) : null;
    r.trade_pay_settled = r.trade_pay_outstanding != null ? r.trade_pay_outstanding === 0 : null;
  }
}
...
async function attachLcs(rows) {
  ...
  const lcs = await selectLcs("WHERE purchase_id = ANY($1) ORDER BY created_at, lc_id", [ids]);
  const byPid = {};
...
// Flag each LC against the bank trade facility (finance.bank_trade_facility):
//   l.on_facility  true / false / null (null = facility unreadable or no real
//                  reference yet) — matched on the LC reference, the reference
//                  Finance keys and HSBC draws under.
...
async function attachOnFacility(lcs) {
  ...
  let byRef = null;
  try {
    const { rows } = await query(`SELECT reference FROM finance.bank_trade_facility`);
    byRef = new Map();
    for (const f of rows) { const n = normRef(f.reference); if (n && !byRef.has(n)) byRef.set(n, f); }
  } catch (e) { if (!absent(e)) throw e; byRef = null; }   // facility table absent → unknown
  for (const l of lcs) {
    if (byRef == null) { l.on_facility = null; continue; }
    const n = normRef(l.lc_reference);
    const match = !NON_REF.has(n) ? byRef.get(n) : null;
    // A real reference that isn't found → false; a placeholder-only LC → null (no nag).
    l.on_facility = match ? true : (!NON_REF.has(n) ? false : null);
  }
}
...
async function attachDcs(rows) {
  ...
  if (!ids.length) return;
  let dcs = [];
  try { dcs = (await query(`SELECT ${await dcCols()} FROM finance.procurement_dc WHERE purchase_id = ANY($1) ORDER BY created_at, dc_id`, [ids])).rows; }
...
export async function setPaymentStatus(id, { payment_status, paid_date, payment_method, trade_pay_ref }, actor) {
  ...
  const method = paid && payment_method ? payment_method : null;
  if (method && !isProcPaymentMethod(method)) throw new Error("Choose a valid payment method");
  /*
   * The drawing this settled on (migration 115). Only meaningful on a trade-pay
   * row, so it is cleared with the method — leaving a stale reference behind on a
   * row switched to cash would reconcile against a drawing that paid for
   * something else.
   *
   * The reference is NOT checked against the facility here. It is flagged on the
   * desk instead: the HSBC extract is uploaded periodically, so a genuine
   * reference may not be loaded yet, and refusing it would stop Finance
   * recording a payment that really happened.
   */
  const tpRef = method === "TRADE_PAY" ? normTradePayRef(trade_pay_ref) || null : null;
  const refErr = tradePayRefError(method, tpRef);
  if (refErr) throw new Error(refErr);
  // Keep the base status column (COMMITTED/PAID) in step for the cash-tracker view.
  const baseStatus = paid ? "PAID" : "COMMITTED";
  await query(`UPDATE finance.procurement_purchase SET payment_status=$2, paid_date=$3, status=$4 WHERE purchase_id=$1`,
    [id, payment_status, paid ? (paid_date || null) : null, baseStatus]);
  // How it settled — Cash or Trade pay (migration 113). Separate best-effort write
  // so the payment update above still works before the column exists. Cleared when
  // the row is no longer paid.
  try {
    await query(`UPDATE finance.procurement_purchase SET payment_method=$2 WHERE purchase_id=$1`, [id, method]);
  } catch (e) { if (e?.code !== "42703") throw e; }
  // Same best-effort shape: works before migration 115 is applied.
  try {
    await query(`UPDATE finance.procurement_purchase SET trade_pay_ref=$2 WHERE purchase_id=$1`, [id, tpRef]);
  } catch (e) { if (e?.code !== "42703") throw e; }
  await audit({ actor, eventType: "procurement.payment", objectType: "procurement_purchase", objectRef: String(id), detail: { payment_status, payment_method: method, trade_pay_ref: tpRef } });
  return { ok: true };
}
...
export async function challengeProcurement(id, { reasons, note }, actor) {
  ...
  // "Other" says nothing by itself — enforce the note here too, not just in the
  // form, so the API can't record an unexplained challenge.
  const noteErr = challengeNoteError(codes, note);
...
// Select LCs with a column fallback (084 → legacy) so the desk keeps working
// before the migration is applied. Returns [] if the table is absent.
async function selectLcs(where, params) {
  for (const cols of [LC_COLS, LC_COLS_LEGACY]) {
    try { return (await query(`SELECT ${cols} FROM finance.procurement_lc ${where}`, params)).rows; }
    catch (e) { if (columnMissing(e)) continue; if (absent(e)) return []; throw e; }
  }
  return [];
}

export async function listLcs(purchaseId) {
  return selectLcs("WHERE purchase_id=$1 ORDER BY created_at, lc_id", [purchaseId]);
}
...
export async function updateLc(lcId, fields = {}, actor) {
  ...
  try {
    ...
    if (columnMissing(e)) throw new Error("A newer LC column is missing — run the outstanding migrations (084 loan fields / 094 customer reference).");
    throw e;
  }
...
export async function addLc(purchaseId, { dc_reference, lc_reference, lc_amount, lc_bank, lc_confirmed_date, lc_payment_date }, actor) {
  ...
  const ref = String(lc_reference).trim();
  const bank = lc_bank || LC_BANK_DEFAULT;
  // Tiered insert so it works before migration 084 is applied.
  const inserts = [
    { cols: "purchase_id, dc_reference, lc_reference, lc_amount, lc_bank, lc_confirmed_date, lc_payment_date, created_by",
      vals: [purchaseId, dc, ref, numOrNull(lc_amount), bank, lc_confirmed_date || null, lc_payment_date || null, actorOf(actor)] },
    { cols: "purchase_id, lc_reference, lc_amount, lc_bank, lc_confirmed_date, lc_payment_date, created_by",
      vals: [purchaseId, ref, numOrNull(lc_amount), bank, lc_confirmed_date || null, lc_payment_date || null, actorOf(actor)] },
  ];
  let done = false;
  for (const ins of inserts) {
    const ph = ins.vals.map((_, i) => `$${i + 1}`).join(",");
    try { await query(`INSERT INTO finance.procurement_lc (${ins.cols}) VALUES (${ph})`, ins.vals); done = true; break; }
    catch (e) { if (!columnMissing(e)) throw e; }
  }
  if (!done) throw new Error("Could not record the LC.");
...
// Migration 114 — when the undrawn balance of a DC is expected to be paid.
// Optional, so a database without 114 keeps working and simply has no month.
const dcCols = () => selectList("finance", "procurement_dc", DC_COLS,
  ["expected_payment_date"]).then((c) =>
  c.replace("expected_payment_date", "to_char(expected_payment_date,'YYYY-MM-DD') AS expected_payment_date"));
const uniqueViolation = (e) => e?.code === "23505";

export async function listDcs(purchaseId) {
  try { return (await query(`SELECT ${await dcCols()} FROM finance.procurement_dc WHERE purchase_id=$1 ORDER BY created_at, dc_id`, [purchaseId])).rows; }
...
export async function addDc(purchaseId, input, actor) {
  ...
  try {
    ...
    // Separate statement, swallowing 42703, so a database without 114 still
    // records the DC rather than refusing it over a column it does not have.
    await setDcExpected(rows[0].dc_id, clean.expected_payment_date);
...
// Best-effort write of the expected payment month (migration 114). Kept out of
// the main statement so the rest of a DC still saves on a database without it.
async function setDcExpected(dcId, date) {
  try {
    await query(`UPDATE finance.procurement_dc SET expected_payment_date=$2 WHERE dc_id=$1`, [dcId, date]);
  } catch (e) { if (e?.code !== "42703") throw e; }
}
...
export async function updateDc(dcId, input, actor) {
  ...
  try {
    ...
    await setDcExpected(dcId, clean.expected_payment_date);
  } catch (e) {
    if (uniqueViolation(e)) throw new Error(`DC reference “${clean.dc_reference}” already exists on this request`);
    throw e;
  }
...
export async function procurementRollup() {
  try {
    const selectPurchases = async () =>
      (await query(`SELECT ${await selectCols()} FROM finance.procurement_purchase`)).rows;
    ...
    // ---- Documentary Credits (Miniso HQ) ----
    // Each DC groups the LCs logged under it in Procurement. The DC value is the
    // total of the LC amounts logged under it (a DC can hold several). The LC value
    // is what Treasury has quoted for those LCs — the amount drawn against them on
    // the HSBC bank trade facility, matched on the LC reference. The balance is
    // DC value − LC value: what's logged in Procurement but not yet quoted/drawn in
    // Treasury. GBP columns convert the LC total at the spot and costing FX rates,
    // with the earliest expected LC payment date.
    const minisoIds = rows.filter((r) => r.source === "MINISO").map((r) => r.purchase_id);
    let dc = { count: 0, currency: "USD", totalValue: 0, totalLcValue: 0, totalBalance: 0, totalSpotGbp: 0, totalCostingGbp: 0, rows: [] };
    if (minisoIds.length) {
      const [dcRecs, lcRecs, facilityRows] = await Promise.all([
        query(`SELECT ${DC_COLS} FROM finance.procurement_dc WHERE purchase_id = ANY($1)`, [minisoIds]).then((r) => r.rows).catch(() => []),
        selectLcs("WHERE purchase_id = ANY($1)", [minisoIds]),
        query(`SELECT reference, loan_amount FROM finance.bank_trade_facility`).then((r) => r.rows).catch(() => []),
      ]);
      const refByPid = {};
      for (const r of rows) refByPid[r.purchase_id] = r.reference || `#${r.purchase_id}`;
      // Aggregate the LCs under each DC: count, USD total (logged), the Treasury-
      // quoted total (facility drawn against each LC, matched on the LC reference),
      // and the earliest expected payment.
      const facIdx = facilityRefIndex(facilityRows);
      const byDc = new Map();
      for (const l of lcRecs) {
        const k = `${l.purchase_id}·${normRef(l.dc_reference || "")}`;
        const cur = byDc.get(k) || { count: 0, usd: 0, quoted: 0, due: null };
        cur.count += 1;
        cur.usd = round2(cur.usd + (l.lc_amount == null ? 0 : Number(l.lc_amount)));
        const fac = lcOnFacility(l, facIdx);
        if (fac && fac.loan_amount != null) cur.quoted = round2(cur.quoted + Number(fac.loan_amount));
        if (l.lc_payment_date && (!cur.due || l.lc_payment_date < cur.due)) cur.due = l.lc_payment_date;
        byDc.set(k, cur);
      }
      const spotRate = findRate(fxRates, "USD", "SPOT");
      const costRate = findRate(fxRates, "USD", "COSTING");
      const dcRows = dcRecs.map((d) => {
        const agg = byDc.get(`${d.purchase_id}·${normRef(d.dc_reference || "")}`) || { count: 0, usd: 0, quoted: 0, due: null };
        const lcTotal = agg.usd;      // DC value = total of the LCs logged under this DC
        const lcValue = agg.quoted;   // LC value = what Treasury has quoted (facility drawn)
        return {
          dc_reference: d.dc_reference, purchaseRef: refByPid[d.purchase_id] || `#${d.purchase_id}`,
          count: agg.count, dc_value: lcTotal, lc_value: lcValue, balance: round2(lcTotal - lcValue),
          gbp_spot: convertToGbp(lcTotal, spotRate), gbp_costing: convertToGbp(lcTotal, costRate),
          due_date: agg.due,
        };
      });
      const totalLc = round2(dcRows.reduce((t, r) => t + (r.dc_value || 0), 0));
      const totalLcValue = round2(dcRows.reduce((t, r) => t + (r.lc_value || 0), 0));
      dc = {
        count: dcRows.length, currency: "USD", totalValue: totalLc,
        totalLcValue, totalBalance: round2(totalLc - totalLcValue),
        totalSpotGbp: convertToGbp(totalLc, spotRate) || 0,
        totalCostingGbp: convertToGbp(totalLc, costRate) || 0,
        rows: dcRows.sort((a, b) => (b.dc_value || 0) - (a.dc_value || 0)).slice(0, 50),
      };
    }
