...
async function attachLcs(rows) {
  ...
  catch (e) { if (absent(e)) { for (const r of rows) r.lcs = []; return; } throw e; }
  const byPid = {};
...
const LC_COLS = `lc_id, purchase_id, lc_reference, lc_amount, lc_bank, lc_confirmed_date, lc_payment_date, lc_settled, lc_settled_date, lc_settled_amount, created_by, created_at`;
...
export async function addLc(purchaseId, { lc_reference, lc_amount, lc_bank, lc_confirmed_date, lc_payment_date }, actor) {
  ...
  await query(
    `INSERT INTO finance.procurement_lc (purchase_id, lc_reference, lc_amount, lc_bank, lc_confirmed_date, lc_payment_date, created_by)
