...
export async function procurementRollup() {
  try {
    ...
    // Each DC groups the LCs logged under it in Procurement; the DC's value is the
    // sum of those LC amounts (USD), shown in GBP at the spot and costing FX rates,
    // with the earliest expected LC payment date. Derived from the LCs — no manual
    // DC value, no facility drawdown.
    const minisoIds = rows.filter((r) => r.source === "MINISO").map((r) => r.purchase_id);
    let dc = { count: 0, currency: "USD", totalValue: 0, totalSpotGbp: 0, totalCostingGbp: 0, rows: [] };
    if (minisoIds.length) {
      ...
      const dcRows = dcRecs.map((d) => {
        ...
        return {
          ...
          count: agg.count, value_usd: agg.usd,
          gbp_spot: convertToGbp(agg.usd, spotRate), gbp_costing: convertToGbp(agg.usd, costRate),
          due_date: agg.due,
        };
      });
      const totalUsd = round2(dcRows.reduce((t, r) => t + (r.value_usd || 0), 0));
      dc = {
        count: dcRows.length, currency: "USD", totalValue: totalUsd,
        totalSpotGbp: convertToGbp(totalUsd, spotRate) || 0,
        totalCostingGbp: convertToGbp(totalUsd, costRate) || 0,
        rows: dcRows.sort((a, b) => (b.value_usd || 0) - (a.value_usd || 0)).slice(0, 50),
      };
    }
