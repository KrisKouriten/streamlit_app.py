...
import { anthropicConfigured, generateText, isLlmConfigError } from "./llm.js";
import { buildCommentaryPrompt } from "./llm-rules.js";
import { notifyUserByName } from "./notifications.js";
import { latestPeriod, getSignals, getOverrides } from "./close.js";
import { evaluatePlan, readinessLabel } from "./close-plan-rules.js";
...
const IMPLEMENTATIONS = {
  ...
  // LLM agent (Tier 2.2). Gathers the governed company-store YTD figures
  // (read-only), then asks Claude to draft a short trading commentary using ONLY
  // those figures. The model gets no tools and no data access — it only turns
  // supplied numbers into prose. The draft is recorded in full and, because the
  // registry sets approval_required, always lands in PENDING_REVIEW for a human
  // to amend or approve. Degrades cleanly to a recorded failure without a key.
  TRADING_COMMENTARY: async (ctx) => {
    if (!anthropicConfigured()) {
      throw new Error("ANTHROPIC_API_KEY is not set — add it as an environment secret to enable LLM commentary.");
    }
    const { rows: win } = await selectOnly(
      `SELECT MAX(d.calendar_date) AS max_date
       FROM commercial.fact_store_sales s
    ...
    const pyFrom = dk(`${year - 1}-01-01`);
    const pyToIso = new Date(new Date(maxIso + "T00:00:00Z").getTime() - 365 * 86400000).toISOString().slice(0, 10);
    ctx.plan(`Summarise company-store YTD trading (01/01/${year} – ${maxIso}) vs last year into a management commentary drafted by Claude — read-only, for human review.`);
    ctx.period(`${year}-01-01`, maxIso);

    const { rows } = await ctx.step("Gather company-store YTD figures vs last year", () => selectOnly(
      `WITH cy AS (
         SELECT st.store_id, st.store_name, SUM(s.net_sales) net, SUM(s.footfall) ff
         FROM commercial.fact_store_sales s
         JOIN core.dim_scenario sc ON sc.scenario_id = s.scenario_id
         JOIN core.dim_store st ON st.store_id = s.store_id
         WHERE sc.scenario_type='ACTUAL' AND st.operator_name IS NOT NULL AND st.ownership_type <> 'OTHER'
       ...
       py AS (
         SELECT st.store_id, SUM(s.net_sales) net, SUM(s.footfall) ff
         FROM commercial.fact_store_sales s
         JOIN core.dim_scenario sc ON sc.scenario_id = s.scenario_id
         JOIN core.dim_store st ON st.store_id = s.store_id
         WHERE sc.scenario_type='ACTUAL' AND st.operator_name IS NOT NULL AND st.ownership_type <> 'OTHER'
           AND s.is_valid_day AND s.date_key BETWEEN $3 AND $4
         GROUP BY st.store_id)
       SELECT cy.store_id, cy.store_name, cy.net, cy.ff, py.net py_net, py.ff py_ff
       FROM cy JOIN py ON py.store_id = cy.store_id`,
      [cyFrom, cyTo, pyFrom, dk(pyToIso)]
    ));

    let cyNet = 0, pyNet = 0, cyFf = 0, pyFf = 0;
    const perStore = [];
    for (const r of rows) {
      const n = Number(r.net) || 0, pn = Number(r.py_net) || 0;
      cyNet += n; pyNet += pn; cyFf += Number(r.ff) || 0; pyFf += Number(r.py_ff) || 0;
      if (pn > 0) perStore.push({ name: r.store_name, yoyPct: n / pn - 1, cyNet: n });
    }
    perStore.sort((a, b) => b.yoyPct - a.yoyPct);
    const figures = {
      period: { from: `${year}-01-01`, to: maxIso }, dataAsOf: maxIso,
      storeCount: rows.length, comparableStores: perStore.length,
      totals: { cyNet, pyNet, yoyPct: pyNet ? cyNet / pyNet - 1 : null },
      footfallYoyPct: pyFf ? cyFf / pyFf - 1 : null,
      movers: { up: perStore.slice(0, 3), down: perStore.slice(-3).reverse() },
    };

    let commentary = "";
    let usage = null;
    await ctx.step("Draft the commentary with Claude (read-only; no tools)", async () => {
      const { system, user } = buildCommentaryPrompt(figures);
      const out = await generateText({ system, user, maxTokens: 1200 });
      commentary = out.text;
      usage = out.usage;
    });

    const yoy = figures.totals.yoyPct;
    ctx.summary(`Commentary drafted from ${perStore.length} comparable stores; YTD net sales ${yoy != null ? (yoy * 100).toFixed(1) + "% vs LY" : "n/a"}${usage ? ` (${usage.output_tokens} tokens)` : ""}. Awaiting review.`);
    return [{
      output_type: "INSIGHT", severity: "MEDIUM",
      headline: `Weekly trading commentary — YTD to ${maxIso}`,
      body: commentary,
      recommended_action: "Review the drafted commentary against the figures, amend if needed, and approve before using it in any pack.",
      confidence_pct: 0.7,
    }];
  },

  // Close automation (Tier 3.1). Reports month-end close readiness for the
  // latest loaded period by evaluating the machine-checkable close gates
  // (lib/close-plan-rules.js) against read-only signals. Emits one status REPORT
  // — not material, so it closes automatically and lands on the close dashboard.
  // It never locks a period or ticks a step; a person does that in the Close
  // Cockpit. Runnable on a schedule so blockers surface without anyone asking.
  CLOSE_STATUS: async (ctx) => {
    const period = await latestPeriod();
    if (!period) throw new Error("No finance actuals loaded — there is no period to close");
    ctx.plan(`Evaluate month-end close readiness for ${period}: actuals, feed freshness, pre-close exceptions, workstream playbook, month-end tasks and trading commentary; report readiness and any blockers.`);
    ctx.period(`${period}-01`, `${period}-01`);

    let plan;
    await ctx.step(`Gather and evaluate close signals for ${period}`, async () => {
      const [signals, overrides] = await Promise.all([getSignals(period), getOverrides(period)]);
      plan = evaluatePlan(signals, overrides);
    });

    const label = readinessLabel(plan);
    const blockerLines = plan.blockers.length
      ? plan.blockers.map((b) => `• ${b.label} — ${b.detail}`).join("\n")
      : "No outstanding blockers.";
    const body =
      `Month-end close for ${period} is ${label.toLowerCase()} (${plan.satisfied} of ${plan.gateTotal} gates satisfied, ${plan.score}%).\n\n` +
      `Outstanding:\n${blockerLines}` +
      (plan.locked ? "\n\nThe period is locked." : plan.ready ? "\n\nAll gates are satisfied — the period is ready to lock." : "");

    ctx.summary(`${period}: ${label}${plan.blockers.length ? ` — ${plan.blockers.length} blocker(s)` : ""}.`);
    return [{
      output_type: "REPORT",
      severity: plan.ready || plan.locked ? "LOW" : plan.blockers.some((b) => b.status === "BLOCKED") ? "HIGH" : "MEDIUM",
      headline: `Close ${period}: ${label} (${plan.score}%)`,
      body,
      recommended_action: plan.ready || plan.locked ? null : "Clear the outstanding blockers in the Close Cockpit, then lock the period.",
      financial_impact: 0,
      confidence_pct: 1,
    }];
  },
...
export async function runAgent(agentCode, actor, triggerType = "MANUAL") {
  ...
  try {
    ...
    for (const o of outputs) {
      ...
      if (lifecycle === "PENDING_REVIEW") pendingReview++;
      await query(
    ...
    // Notify the agent's reviewer that outputs are waiting. Best-effort: a
    // notification failure must never fail the run (notifyUserByName also
    // swallows a missing table pre-migration).
    if (pendingReview > 0) {
      try {
        await notifyUserByName(a.reviewer_name, {
          kind: "agent_review",
          title: `${a.agent_name}: ${pendingReview} output${pendingReview === 1 ? "" : "s"} awaiting your review`,
          body: state.summary || null,
          link: "/ai/review",
          actor: actor.email || actor.name || "agent-runner",
          objectType: "agent_run",
          objectRef: String(runId),
        });
      } catch { /* notification is best-effort */ }
    }
    return { runId, outputs: outputs.length };
  } catch (e) {
    // A missing/rejected Claude API key is a configuration hiccup, not a system
    // fault: record it as a LOW-severity "commentary unavailable" note with a
    // clean message (not the raw 401 JSON), so a key issue never reads as a
    // HIGH agent failure. Genuine data/logic errors stay HIGH with their detail.
    const softConfig = isLlmConfigError(e);
    const severity = softConfig ? "LOW" : "HIGH";
    const message = softConfig
      ? "Commentary unavailable — the Claude API key is missing or invalid. Set ANTHROPIC_API_KEY and re-run."
      : e.message;
    await query(`UPDATE agent.agent_run SET status='FAILED', finished_at=CURRENT_TIMESTAMP, summary=$2 WHERE run_id=$1`, [runId, message]);
    await query(`INSERT INTO agent.agent_exception (run_id, severity, message) VALUES ($1, $2, $3)`, [runId, severity, message]);
    await query(
