...
// A configuration/auth problem (missing or rejected API key) rather than a data
// or logic fault. Callers can classify it — e.g. the agent runner records it as
// a low-severity "commentary unavailable" note instead of a HIGH system fault.
function configError(message) {
  const e = new Error(message);
  e.kind = "LLM_CONFIG";
  return e;
}
export function isLlmConfigError(e) {
  return !!(e && e.kind === "LLM_CONFIG");
}
// Turn a non-2xx Claude response into an error, tagging 401/403 (a rejected or
// missing key) as a config error so it degrades gracefully.
function apiError(status, detail) {
  const raw = `Claude API ${status}: ${String(detail).slice(0, 200)}`;
  if (status === 401 || status === 403) {
    const e = configError("The Claude API key was rejected — check ANTHROPIC_API_KEY.");
    e.detail = raw;
    return e;
  }
...
export async function generateText({ system, user, maxTokens = 1200 }) {
  ...
  if (!key) throw configError("The Claude API key is not set — add ANTHROPIC_API_KEY as an environment secret to enable LLM commentary.");
...
export async function generateGoverned({ system, user, messages = null, model, maxTokens = 4000, schema = null }) {
  const key = process.env.ANTHROPIC_API_KEY;
  if (!key) throw configError("The Claude API key is not set — add ANTHROPIC_API_KEY as an environment secret to enable the intelligence layer.");
