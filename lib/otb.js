...
// ---- Inventory positions ----
// Inventory now lives in the live master (merch.inventory_position), managed on the
// Plan · HO → Inventory Position screen — the OTB process takes inventory over. Every
// OTB version reads the same current position. The versionId parameter is retained
// for call-site compatibility but ignored. Falls back to the legacy per-version table
// only if the master doesn't exist yet (pre-migration 075).
export async function getInventoryPositions(versionId) {
  const master = await safe(async () => (await query(
    `SELECT id, channel_code, location_type, store_code, units, stock_value, reserved_value, damaged_value,
       ...
       FROM merch.inventory_position ORDER BY channel_code, location_type, store_code`)).rows, null);
...
// The consolidated inventory topline OTB shows (stores rolled to one total per
// channel; warehouse net of reserved/damaged; in-transit confidence-adjusted).
export async function getInventoryTopline() {
  return safe(async () => {
    const { rows } = await query(
      `SELECT channel_code, location_type,
              SUM(stock_value) AS gross, SUM(reserved_value) AS reserved, SUM(damaged_value) AS damaged,
              SUM(CASE WHEN location_type = 'STORE' THEN 1 ELSE 0 END) AS store_count,
              MAX(data_through) AS data_through,
              AVG(confidence) AS confidence
         FROM merch.inventory_position GROUP BY channel_code, location_type`);
    return rows;
  }, []);
}
...
// The monthly OTB projection — the Executive summary's whole-horizon pool spread
// across the calendar. Projects each channel month by month from the monthly sales
// plan × the version assumptions (COS %, target stock weeks), with new-store /
// closure / clearance / commitment events allocated to their month, 18 months from
// the version start. Pure maths lives in projectChannelOtb (otb-rules).
export async function getMonthlyOtb(versionId, { scenario = "BASE", horizonMonths = 18 } = {}) {
  const version = await getOtbVersion(versionId);
  if (!version) return { ready: false, months: [], byChannel: {}, total: [] };
  const [sales, assumptions, inv, newStores, closures, clearance, commitments] = await Promise.all([
    getStoreSales(versionId, { scenario }), getAssumptions(versionId), getInventoryPositions(versionId),
    listNewStores(versionId), listClosures(versionId), listClearance(versionId), listCommitments(versionId),
  ]);

  // The horizon starts at the earliest planned sales month, else the fiscal-year start.
  const salesPeriods = sales.map((s) => s.period).filter(Boolean).sort();
  const start = salesPeriods[0] || (version.fiscal_year ? `${version.fiscal_year}-01` : null);
  if (!start) return { ready: true, computed: false, months: [], byChannel: {}, total: [] };
  const periods = periodRange(start, horizonMonths);
  ...
  for (const ch of OTB_CHANNELS) {
    const a = asmByCh[ch] || {};
    // Monthly planned sales for this channel.
    const salesByPeriod = {};
    for (const s of sales) if (s.channel_code === ch && s.period) salesByPeriod[s.period] = (salesByPeriod[s.period] || 0) + Number(s.sales_amount || 0);
    // Opening inventory available at the horizon start (store + warehouse + in-transit).
    const chInv = inv.filter((i) => i.channel_code === ch);
    const openingStock =
      chInv.filter((i) => i.location_type === "STORE").reduce((t, i) => t + Number(i.stock_value || 0), 0) +
      chInv.filter((i) => i.location_type === "WAREHOUSE").reduce((t, i) => t + availableWarehouse({ stockValue: i.stock_value, reservedValue: i.reserved_value, damagedValue: i.damaged_value }), 0) +
      chInv.filter((i) => i.location_type === "IN_TRANSIT").reduce((t, i) => t + inTransitAvailable({ value: i.stock_value, confidence: i.confidence }), 0);
    // Dated events allocated to their calendar month.
    const eventsByPeriod = {};
    const bump = (period, key, amt) => {
      if (!period || !amt) return;
      eventsByPeriod[period] = eventsByPeriod[period] || {};
      eventsByPeriod[period][key] = (eventsByPeriod[period][key] || 0) + amt;
    };
    for (const n of newStores) if (n.channel_code === ch && n.approved) {
      bump(monthKey(n.planned_opening), "newStore", Number(n.opening_stock_value || 0));
      bump(monthKey(n.planned_opening), "fitout", Number(n.fitout_inventory_value || 0));
    }
    for (const c of closures) if (c.channel_code === ch) bump(monthKey(c.closure_date), "closureTransferable", Number(c.transferable_value || 0));
    for (const c of clearance) if (c.channel_code === ch) bump(monthKey(c.start_date), "clearanceReduction", clearanceReduction({ stockValue: c.stock_value, realisationRate: c.realisation_rate }));
    for (const c of commitments) if (c.channel_code === ch && (c.kind === "OPEN_COMMITMENT" || c.kind === "APPROVED_REQUEST")) bump(c.period && c.period !== "ALL" ? c.period : start, "openCommitments", Number(c.amount || 0));

    byChannel[ch] = projectChannelOtb({
      periods, salesByPeriod, cosRate: a.cos_rate, grossMarginRate: a.gross_margin_rate,
      targetWeeks: a.target_stock_weeks, openingStock, eventsByPeriod,
    });
  }

  // The blended total across channels, month by month.
  const total = periods.map((period, i) => {
    let sales = 0, plannedCos = 0, otb = 0;
    for (const ch of OTB_CHANNELS) {
      const row = byChannel[ch]?.months?.[i];
      if (row) { sales += row.sales; plannedCos += row.plannedCos; otb += row.otb; }
    }
    return { period, sales: Math.round(sales * 100) / 100, plannedCos: Math.round(plannedCos * 100) / 100, otb: Math.round(otb * 100) / 100 };
  });
  const totalOtb = Math.round(total.reduce((t, r) => t + r.otb, 0) * 100) / 100;
  return { ready: true, computed: total.some((r) => r.sales !== 0 || r.otb !== 0), start, months: periods, byChannel, total, totalOtb };
}
