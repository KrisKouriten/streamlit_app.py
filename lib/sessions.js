...
// Revoke every active session for a user EXCEPT one (the current one). Used on a
// self-service password change: the user stays signed in here, but any other
// live session is dropped so a changed password takes effect everywhere else.
export async function revokeOthersForUser(userId, keepSid, revokedBy = "self", reason = "password_change") {
  const { rowCount } = await query(
    `UPDATE governance.session
     SET revoked_at = CURRENT_TIMESTAMP, revoked_by = $3, revoked_reason = $4
     WHERE user_id = $1 AND sid <> $2 AND revoked_at IS NULL`,
    [userId, keepSid, revokedBy, reason]
  );
  return rowCount;
}
