...
export async function getActiveSource() {
  // Joiin is the live feed once any of its tables carry rows. The real Joiin
  // data lands in finance.joiin_* (board packs / per-entity P&L), NOT in
  // fact_financials, so probe those — to_regclass guards a table that a given
  // database hasn't migrated yet. fact_financials 'JOIIN' is kept as a fallback.
  const { rows } = await query(
    `SELECT CASE WHEN
        (to_regclass('finance.joiin_boardpack') IS NOT NULL AND EXISTS (SELECT 1 FROM finance.joiin_boardpack))
     OR (to_regclass('finance.joiin_pl_entity') IS NOT NULL AND EXISTS (SELECT 1 FROM finance.joiin_pl_entity))
     OR EXISTS (SELECT 1 FROM finance.fact_financials WHERE source_system = 'JOIIN')
...
export async function getConnectedEntities() {
  ...
  if (src === "JOIIN") {
    ...
    // The connected companies are those actually loaded from Joiin.
    let entities = [];
    try {
      const { rows } = await query(`SELECT DISTINCT entity_name FROM finance.joiin_pl_entity ORDER BY entity_name`);
      entities = rows;
    } catch (e) { if (e?.code !== "42P01") throw e; }
    return { kind: "JOIIN", count: entities.length || meta?.unit_count || 26, entities, label: meta?.label || "Joiin consolidation" };
  }
  const { rows } = await query(
