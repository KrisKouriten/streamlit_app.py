...
// --- Real Xero-sourced finance (Phase 6) ----------------------------------
// Connected entities and their feed status, for the consolidation scope label.
export async function getConnectedEntities() {
  ...
  return { count: rows.filter((r) => r.feed_status === "CONNECTED").length, entities: rows };
}
...
export async function getRealPL() {
  const { rows } = await query(
    `WITH latest AS (SELECT MAX(date_key) AS dk FROM finance.fact_financials WHERE source_system = 'XERO')
     ...
     WHERE f.source_system = 'XERO' AND f.date_key = latest.dk
...
export async function getRealFinanceSnapshot() {
  ...
  const { rows: dk } = await query(`SELECT MAX(date_key) AS dk FROM finance.fact_financials WHERE source_system = 'XERO'`);
  return {
