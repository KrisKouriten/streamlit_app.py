...
// ---- Per-invoice processing status (migration 111) ----
// The workflow each invoice moves through: Received → Processing → Reviewing →
// Paid. This is per-invoice and distinct from the P.O's payment status (which
// rolls up across all invoices). The terminal "Paid" stage stays in step with
// the invoice's paid flag (see purchase-orders.js), so the payment rollup still
// reflects what's actually been paid. Order = workflow order.
export const INVOICE_STATUSES = [
  { code: "RECEIVED", label: "Received", tone: "muted" },
  { code: "PROCESSING", label: "Processing", tone: "amber" },
  { code: "REVIEWING", label: "Reviewing", tone: "amber" },
  { code: "PAID", label: "Paid", tone: "green" },
];
const INVOICE_STATUS_LABEL = Object.fromEntries(INVOICE_STATUSES.map((s) => [s.code, s]));
export const isInvoiceStatus = (code) => INVOICE_STATUSES.some((s) => s.code === code);
// The status object for an invoice row (defaults to Received when unset/unknown).
export function invoiceStatusOf(invoice = {}) {
  return INVOICE_STATUS_LABEL[invoice.invoice_status] || INVOICE_STATUS_LABEL.RECEIVED;
}
 ...
 * an admin can delete it; once signed off, only an admin can. The exception is a
 * challenged P.O — Finance has sent it back to the submitter, so its own submitter
 * may delete it (as well as an admin) rather than having to withdraw it via an
 * admin. Returns { ok, reason }.
 */
export function canDeletePo(po = {}, { isAdmin = false, isOwner = false } = {}) {
  if (isAdmin) return { ok: true };
  // A challenged P.O is back with its submitter to fix or drop — let them delete it.
  if (po.finance_status === "CHALLENGED") {
    return isOwner
      ? { ok: true }
      : { ok: false, reason: "Only the P.O's submitter (or an admin) can delete a challenged P.O" };
  }
  if (isSignedOff(po) || po.finance_status === "CLOSED") {
...
/*
 * A calendar day as 'YYYY-MM-DD', whatever form it arrives in.
 *
 * node-postgres hands a DATE column back as a JavaScript Date at local
 * midnight, not as text. String() of that is "Fri Oct 09 2026 00:00:00 …", so
 * the old String(d).slice(0, 10) produced "Fri Oct 09" — which the P.O invoice
 * rollup then tried to save as a date, and every invoice add with a due date
 * failed. Sorting those strings also ordered dates by weekday name.
 *
 * A Date is read with local getters, because local midnight is what the driver
 * built: toISOString() would shift it a day anywhere east of UTC. Text is
 * trusted only if it already starts with an ISO day. Anything else is null.
 */
...
/*
 * Is a signed-off P.O's invoice late?
 *
 * Finance expect the supplier's invoice within INVOICE_DUE_DAYS of the P.O
 * being approved. Past that, the P.O is worth chasing — with the department
 * head and whoever raised it, who are the people who can get it from the
 * supplier.
 *
 * Returns { state, expectedBy, daysOver }:
 *   state  "received"  an invoice is on the P.O
 *          "waiting"   none yet, still inside the window
 *          "overdue"   none, and the window has passed
 *          "n/a"       not signed off, closed, or no approval date to count from
 *   expectedBy  'YYYY-MM-DD', the approval day plus the window
 *   daysOver    whole days past expectedBy (0 unless overdue)
 * `today` is 'YYYY-MM-DD' or a Date, so tests do not depend on the clock.
 */
export const INVOICE_DUE_DAYS = 15;

export function invoiceChaseStatus(po = {}, today = new Date(), dueDays = INVOICE_DUE_DAYS) {
  const none = { state: "n/a", expectedBy: null, daysOver: 0 };
  if (!isSignedOff(po) || po.finance_status === "CLOSED") return none;
  const approved = isoDay(po.approved_at);
  if (!approved) return none;
  const dayMs = 86400000;
  const utc = (iso) => { const [y, m, d] = iso.split("-").map(Number); return Date.UTC(y, m - 1, d); };
  const expected = utc(approved) + dueDays * dayMs;
  const expectedBy = new Date(expected).toISOString().slice(0, 10);
  const hasInvoice = !!(po.invoice_number && String(po.invoice_number).trim()) || (po.invoice_amount != null && Number(po.invoice_amount) > 0);
  if (hasInvoice) return { state: "received", expectedBy, daysOver: 0 };
  const now = isoDay(today);
  const daysOver = now ? Math.round((utc(now) - expected) / dayMs) : 0;
  return daysOver > 0 ? { state: "overdue", expectedBy, daysOver } : { state: "waiting", expectedBy, daysOver: 0 };
}
