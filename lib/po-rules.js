...
// The payment status derived from the invoices: PAID when all are paid, UNPAID
// when none are, PART_PAID in between. Returns null when there are no invoices
// (the manual status then applies).
export function derivePaymentStatus(invoices = []) {
  const list = Array.isArray(invoices) ? invoices : [];
