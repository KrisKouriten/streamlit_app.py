...
export function validatePo(input = {}) {
  ...
  if (input.payment_value == null || String(input.payment_value) === "" || Number(input.payment_value) <= 0) return "Enter a payment value greater than zero";
  if (input.vat_amount != null && String(input.vat_amount) !== "" && Number(input.vat_amount) < 0) return "VAT amount cannot be negative";
