...
// Trim the key defensively — a trailing newline or stray space from copy-paste
// into the env var is a common cause of Joiin rejecting an otherwise valid key.
function apiKey() {
  return (process.env.JOIIN_API_KEY || "").trim();
}
...
// Balance Sheet for a set of companies as at a month end (YYYY-MM). Joiin's
// Connect API exposes /report/balance-sheet alongside profit-loss; the response
// shares the sections → accounts shape, so joiin-api-map.mapReportRows maps it.
// `elimination` mirrors the P&L consolidation ('eliminate' for the group view).
export async function balanceSheet({ companies, startDate, endDate, currency = "GBP", elimination = "eliminate" }) {
  return post("/report/balance-sheet", { companies, startDate, endDate, currency, eliminationType: elimination });
}
