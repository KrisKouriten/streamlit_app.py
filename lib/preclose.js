...
// Postgres 42P01 = undefined_table: migration 012 has not been run in this
// database yet. Pages render a setup card instead of crashing.
const tableMissing = (e) => e?.code === "42P01";

export async function getPreclose({ monthsCovered = 6 } = {}) {
  try {
    return await getPrecloseUnsafe({ monthsCovered });
  } catch (e) {
    if (tableMissing(e)) return { ready: false, period: null, dk: null, exceptions: [], expectations: [], assured: 0, revenueActual: 0, monthsCovered };
    throw e;
  }
}

async function getPrecloseUnsafe({ monthsCovered = 6 } = {}) {
    ...
    dk
      ? query(
          `SELECT a.account_code, a.account_name, a.natural_sign, SUM(f.amount_gbp) AS amount
           ...
           WHERE f.source_system = $2 AND f.date_key = $1
  ...
  return { ready: true, period, dk, actuals, expectations, ...result, exceptions };
}
